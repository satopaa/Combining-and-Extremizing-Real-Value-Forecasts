...
Even though the forecasting literature agrees that combining multiple predictions of some future outcome outperforms the individual predictions, there is no general consensus about the right way to do this. Ideally, individuals make predictions based on correct information and the aggregator combines the predictions without losing or distorting any of the forecasters' information. This paper analyzes how efficiently the most common aggregators, namely measures of central tendency, use the information in the predictions. The results show, among others, that all weighted arithmetic, harmonic, geometric, quadratic, and cubic means distort the forecasters' information and yield an aggregate that is not consistent with correct information about the future outcome. More generally, no aggregator that remains strictly within the convex hull of the forecasts uses the forecasters' information efficiently as long as the individuals rely on finite information. 
%This class of aggregators contains essentially all measures of central tendency. 
Given that the physical universe only contains a finite amount of information, for all practical purposes, measures of central tendency do not use the forecasters' information efficiently. The paper concludes by discussing two concrete ways to construct efficient aggregators in practice. 
...
%and is often more accurate than the average individual \citep{larrick2006intuitions}. Other  aggregators are different measures of central tendency such as the median, geometric mean, mode, trimmed mean, and so on. Even though it is unclear which procedure should be preferred in general, the forecasting literature by and large agrees that  aggregation of forecasts is beneficial. 
...
%Decision-makers often rely on the advice of experts. For instance, 
In early 2011 the U.S. intelligence believed the terrorist leader Osama Bin Laden to be hiding in a particular Pakistani compound. To decide whether to assault the compound or not, the CIA Director Leon Panetta was consulting a group of intelligence analysts. The agents, however, did not agree on the probability of Osama Bin Laden being in this compound. Their answers ranged from $0.30$  to $0.90$ and even higher values \citep{tetlock2016superforecasting}. 
%Based on such high disagreement, should the military proceed with the assault inside a volatile nation armed with nuclear weapons? 
...
%The idea is that even  only the most accurate forecaster's advice 
would ignore a potentially large amount of relevant information that is being contributed by the rest of the group. What Panetta truly needs is a way to make the decision based on the analysts'  combined information.
%based o n a find a consensus probability that represents their combined information


%\marginpar{Should we begin with something like "Decision makers often seek the advice of field experts"}

Decision-makers in different fields, such as finance, technology, operations, medicine, and others, often consult experts and run into Panetta's problem. The traditional solution is to allow the experts to discuss, share information, and reach consensus through deliberation. Unfortunately, such a consensus is often determined by dominant and not necessarily the most competent individuals, and too much discussion focuses on maintaining the group instead of actually solving the problem \citep{dalkey1968predicting}. In addition, shared information often receives too much weight because this, by the nature of being shared, is mentioned more frequently \citep{stasser1985pooling} but also because anyone with the shared information is perceived as more credible \citep{wittenbaum1999mutual}. As a result, face-to-face discussion typically leads to poorer predictive performance than simply averaging the analysts' initial predictions \citep{dalkey1968predicting}. 


An objective of \textit{collective forecasting} is to reduce the negative effects of group interaction.
%\subsection{Collective Forecasting}
...
%
%These problems are avoided if 
  individuals make predictions in isolation and hence are not influenced by each other. These predictions are then combined 
%This way they do not need to be in the same location and can make their predictions on their own time without knowing anything about the others. 
% Regardless how this is done, t
 with a statistical aggregator that is nothing but a deterministic function from the predictions to  the consensus. 
 ...
 In general, the aggregator's goal is to extract the maximum amount of information from the reported predictions and turn it into a consensus without any loss or distortion of information. If an aggregator achieves this, it is called \textit{efficient}. 
...
%%In general, forecasts can be combined in many different ways.
...
%Other common choices involve different measures of central tendency, such as the median, geometric mean, mode, and trimmed mean, that aim to summarize the predictions with a single central value. Even though it is unclear which option should be preferred in general, the forecasting literature by and large agrees that  aggregation of forecasts is beneficial. 
%%Ever since a large number of simulation and empirical studies have been performed. 
%For instance,  \cite{clemen} discusses $209$ articles on forecast aggregation and concludes that the ``results have been virtually unanimous: combining multiple forecasts leads to increased forecast accuracy." Similarly, \cite{armstrong2001combining}  summarizes a large number of empirical studies and concludes that the aggregate is usually much more accurate than the typical forecast. Therefore it is not surprising that combining forecasts has been common practice among business forecasters for a long time \citep{pokempner1970sales}. 
...
%There are, of course, many different ways one could combine predictions.
%In general, forecasts can be combined in many different ways.
Even though it is unclear how exactly the forecasts should be combined in general, the forecasting literature by and large agrees and has shown much empirical evidence that aggregation of forecasts is beneficial \citep{clemen, armstrong2001combining}. By far the most common aggregators are the (weighted) average, median, mode, or some other measure of central tendency. Their popularity is largely due to their familiarity, simplicity, and good past performance in 
% Panetta could combine the predictions in many different ways. Most people would probably first consider the simple average. 
%has performed well
 a variety of settings \citep{makridakis1983averages,clemen1986combining,clemen1989combining}.
% As long as the forecasts bracket the true outcome, that is, the outcome falls within their convex hull, averaging
...
Measures of central tendency, however, do not always offer an efficient aggregator. To illustrate, 
%suppose two of Panetta's agents relied on different pieces of evidence but both predicted $0.9$. 
suppose a hospital patient has both a magnetic resonance imaging (MRI) and blood test taken.  Fortunately, the results show no signs of poor health. Two doctors then independently analyze the results. The first doctor only looks at the MRI results and predicts that the patient has a probability $0.9$ of being entirely healthy. The second doctor, on the other hand, only looks at the blood test results but also predicts a probability $0.9$ of the patient being entirely healthy. Now the patient has two predictions of $0.9$. 
How should these be combined? Surely, if one looked at both the MRI and blood test results, one would be even more confident of the patient's good health and hence predict a probability somewhat greater than $0.9$.  In this simple scenario, however, all measures of central tendency aggregate to $0.9$. Therefore they are not using the doctors' information efficiently. Even though this is admittedly a very contrived example, it does suggest an interesting question: under what conditions are measures of central tendency efficient?

%\marginpar{Maybe we want to tie to the FBI example}
...
%\subsection{Contributions and Organization} \label{Contributions}

Strikingly, our results show that the answer is: almost never. Our analysis treats the target outcome as random and focuses on univariate outcomes, although the results are likely to hold also in higher dimensions. Importantly, our results do not depend on any particular type or distribution of the target outcome. Therefore the discussion applies to predictions of continuous (Gross Domestic Product, stock prices, product demand), binary (rain or no rain, candidate wins or loses), discrete (number of accidents, goals per season), and many other types. Each prediction is nothing but the expected value of the outcome conditional on the forecaster's personal information. 
%Each prediction is then an expected outcome conditional on the forecaster's personal information. 

Section \ref{propertiesS} begins our discussion by describing the framework of analysis and proving Theorem \ref{optimal}  that lists common properties of all efficient aggregators. In general terms, an aggregator is efficient if and only if there exists some probability model under which it uses all the information in the reported forecasts. This section finishes with a brief discussion of previous work. 
%with the efficient aggregator. In general terms, an aggregator is efficient if and only if there exists some probability model under which it uses all the information in the reported forecasts. 
...
Next, Section \ref{MeasuresOfCentralTendency} developes the inefficiency of the measures of central tendency in a sequence of three steps, each considering an increasingly large subclass of measures of central tendency. The discussion avoids redundancies because there is a trade-off between the size of the aggregator class and how specific the associated results are. For instance, the first step considers only weighted averages but describes their inefficiency in the light of the common properties of all efficient aggregators and also builds intuition for the source of the inefficiency. The last step, on the other hand, considers all measures of central tendency but only shows general inefficiency. Each step leads a separate theorem. These are summarized in the following enumeration.
...
%\item  In general terms, an aggregator is efficient if and only if there exists some probability model under which it uses all the information in the reported forecasts. 
...
A weighted average is called non-trivial if it assigns positive weight to at least two different forecasts.
Theorem \ref{contraction} shows that any non-trivial weighted average violates several of the properties listed in Theorem \ref{optimal} and is therefore inefficient. 
%By non-trivial we mean that the average assigns positive weight to at least two different forecasts.
 More specifically, such a weighted average distorts the forecasters' information and hence leads to sub-optimal predictive performance.

% to suboptimal performance. 
%The section concludes by developing intuition for why weighted averaging is inefficient.  
In the past this result has been believed to arise because the average represents the forecasters' shared information too many times \citep{kim2001inefficiency,palley2015eliciting}. We, however, explain that this is not the reason. 
...
Any linear aggregator, such as the weighted average, faces a trade-off between \textit{under}-representing private information  and \textit{over}-representing the shared information. The efficient strategy balances the trade-off by intentionally over-representing the common information in order to under-represent the private information less. 
%We call this the \textit{shared information tradeoff}.
Unfortunately, the weighted average cannot play this tradeoff. Instead, it always takes an extreme stand and represents shared information optimally but dramatically under-represents private information. 
...
\item Theorem \ref{contractionXphi} considers transformed weighted averages that first apply a strictly monotone transformation to the forecasts, averages them, and then transforms the average back to the original space. First, these aggregators distort the forecasters' information if the transformation is convex or concave and the averaging step is non-trivial.
%positive weight is assigned to at least two different forecasts.
%The results show under mild regulatory conditions that 
%and the average assigns positive weight to at least two different forecasts.
...
Therefore well-known aggregators such as the harmonic, geometric, quadratic, and cubic weighted means are not efficient. Second, under mild regularity conditions, non-trivial transformed weighted averages do not combine non-negative forecasts efficiently. This shows, among others, that it is not efficient to transform probability forecasts to 
% average 
%
%In addition, it is not efficient to average the  
odds, log-odds, or probit scores before averaging them.
% of the probability forecasts themselves. 
Interestingly, \cite{dawid1995coherent} conjectured this result for transformed weighted averaging of two probability forecasts. 
% is inefficient. 
...
\item Even though the above two classes of aggregators are rather general, they fail to capture common measures of central tendency such as the median, midrange, truncated means, and many others. Motivated by this,
...
we analyze aggregators that equal the unanimous forecast if all forecasts agree but stay strictly within the convex hull if some forecasts disagree. No assumptions are made about the structure of the aggregator. Therefore this is a very large aggregator class that contains essentially all measures of central tendency, including the ones mentioned above. Theorem \ref{centralTendency} shows that a member of this class can be efficient only if the forecasters' information sets are allowed to be uncountably large. However, given that in our finite physical universe forecasters use finite information sets, these aggregators and hence all measures of central tendency are not efficient for all practical purposes. 
...
The natural question then to ask is whether an efficient aggregator even exists. Fortunately, the answer is ``yes.'' Even though such construction are not the main focus of this paper, Section \ref{practiceEfficient}  briefly describes two general recipes for deriving efficient aggregators in practice. After this Section \ref{conclusion} mentions some potential future directions for our work and concludes with a general discussion of the results.

Although we refer to information sources for the point forecasts as human experts, our approach is equally valid for any type of information sources, such as models.
...
%%More specifically, it is shown that measures of central tendency are not efficient aggregators of information driven forecasts. Therefore they either distort or lose some of the forecasters' information. 
...
%%Akin to \cite{dawid1995coherent} and  \cite{Ranjan08}, the weights are fixed and sum to one. 
%The results show that weighted averaging is inefficient as long as it assigns positive weight to at least two different forecasts. In fact, such a weighted average is not even based on any correct information. Therefore it distorts the forecasters' information, leading to suboptimal performance. 
%%The section concludes by developing intuition for why weighted averaging is inefficient.  
...
%%\marginpar{It is clear that the average will over count the shared info ration; but what about any convex combination. Perhaps some weighted average? In this paper we show that no measure of central tendency can use the information optimally.}
%
%\item Section \ref{TranscontractionSection} considers transformed weighted averages that first apply a strictly monotone transformation to the forecasts, averages them, and then transforms the average back to the original space. If the averaging step assigns positive weight to at least two different forecasts, inefficiencies arise. The results are split in two parts. First, these aggregators distort the forecasters' information and are inefficient if the transformation is convex or concave. 
%%The results show under mild regulatory conditions that 
%%and the average assigns positive weight to at least two different forecasts.
%%In particular, if the transformation is convex (or concave), then this aggregator is not efficient.  
%% aggregator assigns positive weight to at least two different forecasts, some inefficiency results hold. First, transforming a non-negative forecast to a non-negative value never allows the aggregator to collect information. Second, if the transformation is convex or concave, the aggregator does not collect information.
%Therefore well-known aggregators such as the harmonic, geometric, quadratic, and cubic weighted means are not efficient. Second, under mild regularity conditions, these aggregators do not combine non-negative forecasts efficiently. This shows, in particular, that it is not efficient to transform probability forecasts to 
%% average 
%%
%%In addition, it is not efficient to average the  
%odds, log-odds, or probit scores before averaging them.
...
%Two decades later, the second author, namely the Nobel laureate C. W. J. Granger published an invited review, emphasizing the importance of forecasters' information in both theory and interpretation of forecasts \citep{granger1989invited}. 
...
%%To illustrate,
%% For instance, Leon Panetta should combine the forecasts in a way that utilizes all the information among the intelligence analysts. 
...
%After all, forecasting should be based on information about the future outcome -- not on instinct, random guessing, or gut feel. This observation led to different models of information-driven forecasts. 
%% inherent link between forecasting and information led to several stylized models of information-driven forecasts.
...
% For instance, \cite{granger1989invited} and \cite{kim2001inefficiency} consider models in which each forecaster's information consists in equal parts of public information (known to everyone) and private information (known only to one forecaster). Surprisingly, both authors observe that the simple average of the forecasts is not an efficient aggregator. This result became known as the \textit{inefficiency of the means}. 
...
%  \cite{dawid1995coherent} show that the weighted average of two probability forecasts of a future event is inefficient as long as one forecaster does not know everything that the other forecaster knows.
...
%\cite{Ranjan08} generalize this result to $N \in \mathbb{N}$ probability forecasts.
...
%These results are highly general because they do not assumes anything about the forecasters' information structure or the joint distribution of the outcome and its information-driven forecasts. The only assumption is that the outcome and the forecasts operate under a common probability model.
%
%%Given that neither work assumes nothing about the underlying probability model except its existence, their results are extremely general and hold jointly across all probability models and information structures.
%%This makes their results very general. In particular, the inefficiency of the means is more general than initially suggested by \cite{granger1989invited}.
%%Their results are very general because they do not restrict the forecasters' information structure or 
...
\subsection{Forecasts and Outcome} \label{outcomes}
Suppose there are $N \in \mathbb{N}$ forecasters making point predictions $X_1, \dots, X_N$ about some future outcome $Y \in \R$ defined on a probability space $(\Omega, \F , \P)$. In general, assuming the existence of such a space cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].'' For technical reasons, the outcome is assumed to be well-behaved in a sense that is has finite first two moments. Denote its mean and variance with $\E(Y) := \mu_0 < \infty$ and $\Var(Y) := \delta_0 < \infty$. This all gives our first modeling assumption of the article:
...
\begin{customthm}{1}
The outcome $Y \in \R$ is defined under some probability space $(\Omega, \F , \P)$ and $Y \in \mathcal{L}^2$, where $\mathcal{L}^p  := \{ X \in \F : \E[|X|^p] < \infty\}$ is the set of all random variables $X \in \F$ with finite $p$th moment. 
\end{customthm}
...
Each forecaster makes the prediction based on some information about $Y$. This can be incorporated into the analysis with $\sigma$-fields; see \citealt{herves2013information} for a thorough discussion on why $\sigma$-fields are an appropriate description of information. In particular, the principal $\sigma$-field $\F$ represents all possible information that can be known about $Y$. The $j$th forecaster, however, knows only some subset  $\F_j \subseteq \F$ of this information. No assumption as to how the forecaster acquires this information is needed. For instance, it could  stem from books, news articles, personal recollections, and so on. These details are not relevant for aggregation and hence can be abstracted away. 
...
Based on this information the $j$th forecaster has a conditional distribution $\P(Y|\F_j)$ of $Y$.  The forecaster then converts this distribution into a point prediction $X_j$, depending 
...
on the loss function $L(X,Y)$ that is used for evaluating the accuracy of the prediction. Similarly to many previous forecasting applications this article uses \textit{Bregman loss} \citep{banerjee2005optimality} defined as follows: if $\phi: \R \to \R$ is a strictly convex, differentiable function, then the Bregman loss function is $L(x,y) = \phi(x)-\phi(y) - (x-y)\phi'(y)$. The most well-known member of this class is the squared error loss $L(x,y) = (x-y)^2$ given by $\phi(x) = x^2$. In general, if $\phi$ is continuously differentiable, then Bregman loss functions are precisely those functions whose expected value is minimized by $\E(Y|\F_j)$ among all $\F_j$-measurable random variables, i.e.,  $\argmin_{X \in \F_j} \E[L(X,Y)] = \E(Y|\F_j)$. Therefore, under Bregman loss, the $j$th forecaster predicts $\E(Y|\F_j)$. 
%
...
%
%This way the forecasts consist of both signal and noise. The aggregation challenge is to use all the information in the signals. First, however, it necessary to remove the noise from the prediction. This can be achieved via calibration. 
%the forecasts use different information but also make mistakes in interpreting this information. In order to aggregate the information in the predictions, it is necessary to filter the true 
% Even though this is a very likely description of real-world forecasts, the two sources of variation cannot be separated and hence the framework cannot be applied directly. For this reason, past literature has developed aggregators by holding either variation constant. Historically, the most common choice is the measurement error framework. 
...
%
%%Perhaps because of these success stories, forecasts are often aggregated with some measure of central tendency even when the measurement framework is not a reasonable description for the application at hand. 
...
In practice the forecasters may not use all the information they know about $Y$. For instance, they may not be able to  reconcile all the available information or simply may decide to conceal some of it for various reasons. In the end of the day, the aggregator only gets to operate on the reported forecasts and the
% The aggregator only operates on the reported forecasts and hence can access only
  information these forecasts reveal. Therefore, for the sake of aggregation, any unused information may as well not exist, and it is in no way restrictive to set $\F_j = \sigma(X_j)$ and hence write $X_j = \E(Y | X_j)$.
  
  
This is a common assumption in economics where forecasts of the above form are called rational or honest (see, e.g., \cite{keane1990testing, ottaviani2006strategy}). In the statistics and meteorological forecasting, on the other hand, they are called \textit{calibrated} (see, e.g., \citealt{dawid1995coherent, Ranjan08, jolliffe2012forecast}). To illustrate, consider a calibrated probability forecast. If it equals $p$, then the outcome happens $100p\%$ of the time. This is precisely how probabilities are generally understood. Most forecasters with a basic understanding of probabilities are likely to target good calibration even without being instructed to do so. In fact, researchers have found many calibrated subpopulations of experts, such as meteorologists \citep{murphy1977reliability}, experienced tournament bridge players \citep{keren1987facing}, bookmakers \citep{dowie1976efficiency}, and  economic forecasters \citep{keane1990testing}. On the more theoretical side, \cite{dawid1982well} show that an internally consistent or \textit{coherent} Bayesian can expect to become calibrated under feedback. \cite{foster1998asymptotic}, on the other hand, show that the forecaster needs to know almost nothing about the outcome in order to be calibrated. 
...
%\marginpar{There are of course many different ways the forecaster could report the information Y | F. WE have decided to work with calibration because there is much literature on it and poeple know how to improve achieve or improve it in many different context.}
 ...
 If, however, one is left with uncalibrated forecasts, they can be calibrated in different ways. If the forecaster has made predictions in the past such that a stream of realized outcome-forecast pairs $(y_k, x_k)$ for $k \in \mathbb{N}$ is available, a new forecast $x$ can be empirically calibrated by replacing it with the average of $\{y_k : x_k = x, k \in \mathbb{N}\}$. If no past observations are available but one can construct a reasonable joint distribution $f(y,x)$ for the forecast-outcome pairs, then a new forecast $x$ can be calibrated by replacing it with $\E(Y|X = x)$ under the chosen joint distribution. In the most challenging case with no past observations or expertise to construct a joint distribution,
...
Given that our notion of calibration depends on the probability measure $\P$, it may seem  different from the widely studied notion of empirical calibration (\citealt{dawid1982well, foster1998asymptotic}; and many others). 
However, as was pointed out by \citet{dawid1995coherent}, these two definitions of calibration can be expressed in formally identical terms by letting $\P$ represent the limiting joint distribution of the outcome-forecast pairs. 
...
They are defined as any forecasts that are measurable with respect to $\F'' := \sigma(X_1, \dots, X_N)$, namely the $\sigma$-field revealed by the forecasts or, equivalently, information generated by the forecasters' information sets $\F_j$, $j = 1, \dots, N$.  
%Figure \ref{CollectInfo} gives a simple illustration of the relationship between $\F$, $\F''$, and $\F_j$ for $N = 3$ forecasters. 
Intuitively, this includes all information that can be inferred from the reported predictions. An aggregator is efficient if it uses all of this information under some probability model.
...
\noindent
Overall, this is definition places rather weak conditions for efficiency: there has to be at least \textit{one} probability space under which the given aggregator is efficient. On the flip side, failing to meet these conditions suggest a \textit{strong form of inefficiency}: it is impossible to construct a probability space under which the given aggregator is efficient. 

In regression analysis, the efficient aggregator $\mathcal{X}''$ is often called the \textit{regression function}. It is considered the \textit{best predictor} of $Y$ (see, e.g., \citealt[Theorem 6.3.1]{christensen2011plane}), where ``best'' means minimum expected Bregman loss. For this reason, most regression procedures aim to find or estimate $\mathcal{X}''$. 
...
The next theorem enumerates several common properties of all efficient aggregators.
...
\item $\mathcal{X}''$ is \textit{calibrated}: $\E(Y|\mathcal{X}'') = \mathcal{X}''$. \label{calibrationEfficient}
\item $\mathcal{X}''$ is \textit{expanding}: $\max ( \Var(X_1), \dots, \Var(X_N)  ) \leq \Var(\mathcal{X}'')$. 
...
To interpret the rest of Theorem \ref{optimal}, consider
...
an increasing sequence of $\sigma$-fields $\F_0 = \{\emptyset, \Omega\} \subseteq \F_1 \subseteq \dots \subseteq \F_R \subseteq \F$ and the corresponding forecasts $X_r = \E(Y|\F_r)$ for $r = 0,1, \dots, R$. By \citealt[Proposition 2.1.]{satopaamodeling2}, the variances of these forecasts follow the same order as their nested information sets, that is, $\Var(X_0) = 0 \leq \Var(X_1) \leq \dots \leq \Var(X_R) \leq \Var(Y) = \delta_0$. Thus the amount of information in a calibrated forecast is measured by its variance. Naturally, if an aggregator is efficient, it uses all of the forecasters' information and hence uses at least as much information as the most informed individual among them. Therefore the efficient aggregator $\mathcal{X}''$ is necessarily at least as variable as each of the individual forecasts. This is precisely what item iii) describes. 
...
This led to different stylized models of information-driven or, equivalently, calibrated forecasts. 
% inherent link between forecasting and information led to several stylized models of information-driven forecasts.
%These models typically treat the outcome as a random variable and each prediction as a different expectation of this outcome, conditioned on the forecaster's information. 
%The main modeling differences are in the assumptions made about the structure of the forecasters' information.
 For instance, \cite{granger1989invited} and \cite{kim2001inefficiency} consider models in which each forecaster's information consists in equal parts of shared (known to everyone) and private  (known only to one forecaster) information about the outcome. 
% \cite{figlewski1983rational} analyze a model that groups the forecasters into two types, depending on their information. 
 Both authors find the simple (equally weighted) average to be inefficient. This result became known as the \textit{inefficiency of the means}.
...
Unfortunately, these studies rely on strict structural assumptions about the forecasters' information and hence do not imply the strong form of inefficiency discussed in Section \ref{InfoCollection}. In reality the forecasters can have different amounts of information, and not all information is strictly shared or private. For instance, two forecasters may share information that a third forecaster does not know and so on. 
% Therefore different parts of information can be shared by different subgroups of the forecasters, and 
This way the forecasters' information can be seen to form a very complex structure of partially overlapping sets. 

Even though analyzing all such possible information structures may at first appear combinatorially intractable,
%Previous work, however, has shown this to not be the case.
%Some authors, however, have succeeded in this.
% shown that this is not the case. 
%In fact, some authors have analyzed the average forecast without restricting the forecasters' information structure. 
%This, however, is not the case.
  some previous authors have carried out such exhaustive analyses. Under such generality,
  \cite{dawid1995coherent} show that the weighted average of two calibrated probability forecasts of a binary outcome is inefficient as long as one forecaster does not know everything that the other forecaster knows.
%This result, however, is not directly useful for many real-world forecasting applications because these often involve a large number of forecasters.
\cite{Ranjan08} extend the result to $N \geq 2$ calibrated probability forecasts of a binary outcome. This is the current state of the literature. In other words, to the best of our knowledge, the most general result in this line of research shows that the weighted average of calibrated probability predictions of a binary outcome is inefficient. 
%These authors also make no assumptions about the underlying probability model. 
%Therefore the results are very general. 
%Even though these authors do not focus on the role of information, their forecasters can be shown to be operating on different information sets. The structure of these information sets, however, is in no way restricted.  Instead the results are derived jointly under all possible information structures. 
%In addition to not restricting the information structure,
%These results are highly general not only because they consider all possible information structures but also because they do not assume anything about the joint distribution of the outcome and its information-driven forecasts. 
%The only assumption is that the outcome and the forecasts operate under a common probability model.
...
Our work now contributes to this understanding by showing that the inefficiency does not depend on the outcome being binary. In fact, the result holds under any type of univariate outcome and possibly even at higher dimensions. We also  show that the inefficiency is not limited to weighted averaging but instead applies to a much larger class of aggregators, namely the measures of central tendency. This way the story about inefficiency becomes one of central tendency.
...
%\marginpar{We explain that the story is not specific about probability forecasts or weighted averaging. Instead the story is much more general than that. In fact, despite the outcome, the story is about central tendency. Make this seems much more general!}
...
The weighted average is by far the most common way to combine predictions. In fact, even some more complex aggregators often incorporate the weighted average as a sub-component (see Section \ref{TranscontractionSection}).
% In practice most aggregators rely the weighted average of the individual predictions.
  Therefore it is the natural starting point for our analysis of measures of central tendency. The weighted average is defined and notated as follows.
% More precisely, the focus is on the following class of aggregators.
\begin{class}[Weighted Averages] \label{Xw}
The weighted average is $\mathcal{X}_w := \sum_{j=1}^N w_jX_j$, where each weight $w_j \geq 0$ is fixed and $\sum_{j=1}^N w_j = 1$. The weighted average $\mathcal{X}_w$ is non-trivial if there exists a forecast pair $i \neq j$ such that $\P(X_i \neq X_j) > 0$ and $w_i, w_j > 0$.
\end{class} 
...
Overall, this class is large yet specific enough for detailed and useful results. These are given in the following theorem that describes $\mathcal{X}_w$ in the light of the efficiency properties listed in Theorem \ref{optimal}. Note that any deviation from those properties indicates inefficiency. 
...
\item A non-trivial $\mathcal{X}_w$ is inefficient.
\end{enumerate}
\end{theorem}
...
According to item \ref{XwCalib}), a non-trivial $\mathcal{X}_w$ is never calibrated. Therefore it is not based on any correct information about $Y$ even though the forecasters only give it correct information. This means that it somehow distorts the forecasters' information. 


Items \ref{underconfA}) and \ref{underconfB}) show that the distortion contracts $\mathcal{X}_w$ towards $\mu_0$, making it under-confident. The two items do this comparison under different benchmarks. Item \ref{underconfA}) defines and considers under-confidence relative to the calibrated version of  $\mathcal{X}_w$. Intuitively, this aggregator, called $\mathcal{X}_w''$, uses what ever undistorted information is left in $\mathcal{X}_w$. Given that $\mathcal{X}_w$ is closer to $\mu_0$ than $\mathcal{X}_w''$ is, the distortion contracts $\mathcal{X}_w$ towards $\mu_0$. 
...
Item \ref{underconfB}), on the other hand, defines under-confidence relative to $\mathcal{X}''$. This checks whether $\mathcal{X}_w$ is as confident as it should be, given the information it received through the forecasts. Unfortunately, $\mathcal{X}_w$ is under-confident unless it assigns all the weight  to a forecaster whose information set contains every other forecasters' information. Even if $\mathcal{X}_w$ could pick out such a superior forecaster ex-ante, the chances of a single forecaster knowing everything that the rest of the forecasters know is extremely small in practice. In essentially all other cases, $\mathcal{X}_w$ is not expanding or calibrated, and hence cannot equal $\mathcal{X}''$. Therefore it does not use the forecasters' information efficiently. 

%\marginpar{View the weighted average as a linear function of inforamtion} 

This inefficiency of the means is sometimes believed to arise from the fact that $\mathcal{X}_w$ represents the shared information too many times \citep{kim2001inefficiency, palley2015eliciting}. The following example, however, shows that it does not arise from over-representing  shared information but from under-representing private information. Because of this, the average uses too little information and appears under-confident. 
...
%This is explored in the following example.
 For the reader's convenience all of its technical derivations have been deferred to the Appendix.
% goes wrong with direct averaging of forecasts. 

\begin{example}\label{intuition}
%Without loss of generality, let $\Var(Y) = 1$. 
For intuition it suffices to consider only $N = 2$ predictions.  To understand how aggregators represent the private and shared information, the example must explicitly incorporate both types of information without making too many extra assumptions. To do this, consider the three-level hierarchy shown in Figure \ref{HierarchyPlot}. In the bottom of the hierarchy are three \textit{interns} who make predictions about $Y$. Suppose their predictions $X_{(1)}, X_{(1,2)}, X_{(2)}$ are independent. Therefore the interns' information sets $\sigma(X_{(1)})$,  $\sigma(X_{(1,2)})$,  $\sigma(X_{(2)})$ are independent and hence informationally ``disjoint.'' On the second level are two experts. Their private and shared information are determined by the interns' predictions. More specifically, for $j = 1,2$, the $j$th expert predicts $X_j = \E(Y|X_{(j)}, X_{(1,2)})$, where $X_{(j)}$ is the $j$th forecaster's private information and $X_{(1,2)}$ is the forecasters' shared information. In the end, the experts report their predictions $X_1$ and $X_2$ to the decision-maker who combines them with the weighted average $\mathcal{X}_w$. The objective is to explore how $\mathcal{X}_w$ represents the experts' different parts of information, namely $X_{(1)}, X_{(1,2)}, X_{(2)}$. 


%\marginpar{Why call them interns. Just let them be independent pieces of information but explain that they have the form of a forecast.}

%For intuition it is sufficient to consider only $N = 2$ forecasters. 
%
%
%Therefore consider three independent random variables $Z_{(1)}, Z_{(2)}$, and $Z_{(1,2)}$. Let $\sigma(Z_{(j)})$ be the $j$th forecaster's private information and $\sigma(Z_{(1,2)})$ be their public information. Construct the corresponding forecasts $X_{(1)} = \E(Y|Z_{(1)}), X_{(2)} =\E(Y|Z_{(2)})$, and $X_{(1,2)} = \E(Y|Z_{(1,2)})$. These are calibrated and therefore 
%
...
If the weighted average $\mathcal{X}_w$ is efficient, this must occur some probability model where the efficient aggregator is linear, that is,  $\mathcal{X}'' = \beta_1X_1 + \beta_2X_2$ for some constants $\beta_1, \beta_2 \in \R$.  
...
If $\delta_j := \Var(X_j)$ and $\rho := \Cov(X_1, X_2)$, then minimizing the expected Bregman loss (such as the quadratic loss) gives
 ...
 $\beta_j = (\delta_1\delta_2 - \rho \delta_i)/(\delta_1\delta_2 - \rho^2) \geq 0$, where $j,i \in \{1,2\}$ and $j \neq i$. This shows that if the experts' information sets $\F_1$ and $\F_2$ are independent,  then $\mathcal{X}'' = X_1 + X_2$, that is, independent information combines via direct summing. To give intuition for this, consider two real-estate agents $A$ and $B$ who are valuing an apartment. Agent $A$ only sees one room and based on it values the apartment at $\$ x_A$. Agent $B$, on the other hand, sees a different room and  gives a valuation of $\$x_B$. If the agents know nothing else about the apartment except the rooms they visited, their combined information gives a valuation of $\$(x_A + x_B)$. 
%  combined 
 
 
 Now, given that the interns' information sets are independent, the $j$th expert's prediction can be written as $X_j = X_{(j)} + X_{(1,2)}$. This allows us express the aggregators in terms of private and shared information: 
 \begin{align}
...
\end{align}
%This shows 
Both aggregators assign the same proportion of weight to private and shared information. The only difference is that the weights in $\mathcal{X}''$ are not sum-constrained. 
%in the sum of the weights.
In fact, $\beta_1 + \beta_2 \geq 1$ grows monotonically as $\rho$ decreases.
%, reaching $1$ when $\rho = \min(\delta_1,\delta_2)$.  
Therefore $\mathcal{X}''$ classifies as a weighted average only when one of the expert's information is entirely contained within the other expert's information set. This happens if $X_{(1)}$, $X_{(2)}$, or both are identically zero. The corresponding coefficients $(\beta_1, \beta_2)$ are $(0,1)$, $(1,0)$, and $(1/2, 1/2)$, respectively.  Given that in the third case the forecasts are equal, in none of these cases $\mathcal{X}''$ is a non-trivial weighted average. Therefore this observation aligns with item \ref{underconfB}) of Theorem \ref{contraction}. In all other cases there is partial information overlap, $\beta_1 + \beta_2 > 1$, and hence $\mathcal{X}_w$ could be improved by relaxing its sum-constraint. 
...
To understand why this would help,  observe that the optimal way to combine the three pieces of information, that is, the interns' predictions is via simple addition:
 \begin{align}
\mathcal{X}' := \E(Y|X_{(1)}, X_{(2)}, X_{(1,2)}) &= X_{(1)} + X_{(2)} + X_{(1,2)}.\label{X_pDecom}
\end{align}
This aggregator uses all the information in the hierarchy and therefore cannot be improved upon. Unfortunately, the decision maker only has access to the experts' predictions and hence cannot separate out the three pieces of information in this manner.
...
% are typically not available in practice due to company confidentiality or the forecasters' inability to identify the knowledge leading to their opinions.
  Therefore $\mathcal{X}'$ is not fully attainable.  It does, however, offer a theoretical optimum, and every aggregator should aim to be as close to it as possible. 
...
% \marginpar{When is $\beta$ negative?}
 ...
 Comparing (\ref{X_pDecom}) to (\ref{X_wDecom}) shows that $\mathcal{X}_w$ weights the shared information exactly as it should but under-weights any private information. Of course, if there is no shared information, $\mathcal{X}_w$ dramatically under-weights all information. Comparing  (\ref{X_pDecom}) to (\ref{X_ppDecom})  shows that $\mathcal{X}''$ cannot equal $\mathcal{X}'$ under any $\beta_1$ and $\beta_2$. Therefore some over- or under-weighting of information is unavoidable. If $\beta_1 + \beta_2 > 1$, there is partial information overlap and $\mathcal{X}''$ loses due to over-weighting of the shared information but gains due to less under-weighting of the private information. Therefore choosing an appropriate values for $\beta_1$ and $\beta_2$ is equivalent to making a good trade-off between over-weighting shared and under-weighting private information.  The efficient aggregator balances this tradeoff
% , that we call the \textit{public-private information tradeoff},
% cannot be avoided by any linear aggregator of the forecasts. 
...
in a way that brings it as close to the theoretical optimum $\mathcal{X}'$ as possible. In fact, $\mathcal{X}''$ is the orthogonal projection  of $\mathcal{X}'$ onto the space of linear aggregators.
%The above choice $\gamma$ indeed minimizes $\E [(\gamma X_1 + \gamma X_2 - \mathcal{X}')^2]$. 
Therefore, perhaps somewhat counter-intuitively, intentionally over-weighing shared information is beneficial. The average can never do this. Instead it makes an extreme choice on the public-private information tradeoff and remains inefficient. 

 
% 
% 
% A similar result can be shown to hold for any number of independent forecast. In particular, 
% 
% 
% 
...
%
%
%To understand why efficiency needs a bigger ``pie size,'' it is helpful to analyze how $\mathcal{X}_w$ and $\mathcal{X}''$ treat public and private information. Of course, such an analysis must be able to decompose the forecasts $X_1$ and $X_2$ into their private and public subcomponents. 
%% must be 
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \path[every node/.style={font=\sffamily\small}]
    ...
    (5) edge node [right] {} (6);
\end{tikzpicture}
   \caption{Illustration of the three-level hierarchy used in Example \ref{intuition}. The variables $X_{(1)}, X_{(2)}, X_{(1,2)}$ represent the first forecaster's private information, the second forecaster's private information, and their shared information, respectively.}
   \label{HierarchyPlot}
\end{minipage}
\hspace{0.7em}
%\begin{subfigure}[t]{.48\textwidth}
\begin{minipage}[t]{.48\textwidth}
   ...
   \caption{A comparison of the simple average with with the efficient aggregator. Full information consists of five pieces. From left to right, Yellow knows the second and third; Green knows the second and final two; Blue knows the last and first two.  }
   \label{Intuition}
%\end{subfigure}
\end{minipage}
\end{figure}
...
The intuition developed in Example \ref{intuition} holds in general because private and shared information cannot be separated from the reported forecasts. Therefore any weight assigned to $X_j$ is automatically assigned to both its private information and any information it shares with other forecasts. 
%This way the shared information receives at least as much weight as any private information. 
Unlike the weighted average, an unconstrained linear aggregator can play with the tradeoff and move closer to the hypothetical optimum $\mathcal{X}'$.  Figure \ref{Intuition} illustrates the tradeoff under $N = 3$ forecasters, each indexed by a different color. The height of the rectangle is the weight given to the corresponding forecaster, and each rectangle of the same color must have the same size. 
...
%
%This section focused on the weighted arithmetic mean of the forecasts. There are, however, many different types of averages such as the harmonic and geometric means that can be more relevant in some applications. 
...
\subsection{Transformed Weighted Averages} \label{TranscontractionSection}
...
The beginning of Section \ref{contractionSection} mentions that the weighted average is often a sub-component of the overall aggregator. This section explores such aggregators by expanding  Definition \ref{Xw} to allow transformations of the forecasts before they are averaged. Typically the goal is to transform the forecasts $X_j$ to some other space where they are more symmetric around their mean \citep{baron2014two}. This mean is often interpreted as the ``true'' prediction or a prediction made by an ``ideal'' forecaster. Once the forecasts have been transformed, the true prediction can be estimated through averaging. The final step is to transform the average back to the original space for decision making. These aggregators, called transformed weighted averages, are defined and notated as follows. 
\begin{class}[Transformed Weighted Averages] \label{Xphi}
The transformed weighted average  is $\mathcal{X}_{\Phi} := \Phi^{-1} \left[ \sum_{j=1}^N w_j \Phi(X_j) \right]$, where $\Phi$ is some invertible function, each weight $w_j \geq 0$ is fixed, and $\sum_{j=1}^N w_j = 1$. The transformed weighted average $\mathcal{X}_\Phi$ is non-trivial if there exists a forecast pair $i \neq j$ such that $\P(X_i \neq X_j) > 0$ and $w_i, w_j > 0$.
\end{class} 
...
Suppose $\Omega = \{\clubsuit, \heartsuit, \diamondsuit, \spadesuit\}$ and let $\F = 2^\Omega$ be all subsets of $\Omega$. The outcome is $Y = \one_A$, where $A = \{\heartsuit, \spadesuit\}$, and $\P$ is uniform over $\Omega$, that is, $\P(E) = |E|/4$ for all $E \in \F$. Consider two forecasters with the information sets $\F_1 = \sigma(\{\clubsuit\}, \{\heartsuit\})$ and $\F_2 = \sigma(\{\diamondsuit\}, \{\spadesuit\})$. In other words, the first forecaster knows whether the suit is Clubs or Hearts. The second forecaster, on the other hand, knows  whether the suit is Diamonds or Spades. Thus no forecaster is redundant, and their joint information is more than either of the individual information sets. In fact, together they know everything $\F'' =  \sigma(\{\clubsuit\}, \{\heartsuit\}, \{\diamondsuit\}, \{\spadesuit\}) =  \F$. For each $\omega \in \Omega$, the relevant predictions are
\begin{align*}
...
Now, consider $\mathcal{X}_\Phi$ with $\Phi(x) = \logit(x) =  \log(x / (1-x))$. This is a continuous monotone  function that gives $w\logit(X_1) + (1-w)\logit(X_2) = \logit(Y)$ for all $w \in (0,1)$.  The identity holds because one of the log-odds always equals to $\logit(Y) \in \{-\infty, \infty\}$ while the other one is zero. Multiplying either value by a constant has no effect, and $w$ could as well be removed from the aggregator. After transforming the average log-odds back to the probability space, the aggregator 
...
Suppose that $\Phi$ is monotone and that  $\Phi(X_j) \in \mathcal{L}^2$ for all $j = 1, \dots, N$. Then a non-trivial $\mathcal{X}_\Phi$ has the following properties.
% If $\mathcal{X}_\Phi$ assigns positive weight to at least two different forecasts,  then the following holds.
\begin{enumerate}[i)]
\item If $\Phi$ is strictly convex or concave,  $\mathcal{X}_\Phi$ is not marginally consistent: $\E[\mathcal{X}_\Phi] \neq \mu_0$.
\item If $\Phi$ is convex or concave, $\mathcal{X}_\Phi$ is not calibrated, $\P(\E[Y | \mathcal{X}_\Phi] \neq \mathcal{X}_\Phi) > 0$, or efficient.  \label{convex} 
...
\item If  $X_j \geq 0$ for all $j = 1, \dots, N$ and $\Phi^{-1}$ has a bounded first derivative, $\mathcal{X}_\Phi$ is not efficient. 
...
% $\frac{\partial }{\partial w_j} \E\left(\mathcal{X}_\Phi^2\right) = \E\left(\frac{\partial }{\partial w_j} \mathcal{X}_\Phi^2\right)$. Bounded first derivative, however, is not a necessary condition for this interexchange. More generally, $\mathcal{X}_\Phi$ is not an efficient aggregator of non-negative forecasts if there exists some integrable random variable $Z$ such that $|\frac{\partial }{\partial w_j} \mathcal{X}_\Phi^2| \leq Z$ almost surely. Item \ref{mono}), however, was designed specifically for probability forecasts. For this purpose, bounded first derivative is enough to extend the inefficiency result to all the cases that are commonly used in practice. Therefore the theorem statement is kept clean of unnecessary technical details.
\end{theorem}
...
Unfortunately, by item \ref{mono}) of Theorem  \ref{contractionXphi},  these and many other CDFs do not yield an efficient $\mathcal{X}_\Phi$. Therefore averaging the odds, log-odds, or probit scores instead of the probabilities does not make the aggregator efficient.
...
\subsection{Random Convex Combinations}\label{RandomSection}
Even though the classes of aggregators discussed so far are rather large, they do not capture many well-known measures of central tendency, such as the median, midrange, or trimmed averages.  In short, this is because such aggregators do not treat the forecasts symmetrically ex-ante but instead decide how the forecasts are treated after observing their realized values. 
...
The key towards a fully general analysis is to notice that all measures of central tendency are constrained to the convex hull of the forecasts. This property defines a new aggregator class called random convex combinations.

%\marginpar{Explain that this class can be viewed as weighed averages where the weights depend on the realized values of the forecasts.}

\begin{class}[Random Convex Combinations] \label{Xc}
The random convex combination $\mathcal{X}_{[\cdot]}$ remains within the convex hull of the forecasts, that is, $\mathcal{X}_{[\cdot]} \in [\min(X_1, \dots, X_N), \max(X_1, \dots, X_N)]$ almost surely. The subindex here reminds us that $\mathcal{X}_{[\cdot]}$ can take on any value within the closed convex hull. 
\end{class} 
...
Observe that this definition does not specify the functional form of the aggregator. In particular,  the aggregator can decide how to treat the forecasts before or after they have been reported. The only requirement is that it remains within the convex hull under any realization of the forecasts.  Therefore this is clearly the most general class of aggregators considered so far in this paper. Most importantly, it contains all measures of central tendency. Unfortunately, however, the following example shows that the class of random convex combinations is slightly too large for a general inefficiency result.
% This is illustrated in the following example. 
%Fortunately, the class can be modified slightly without excluding any measures of central tendency. 
\begin{example}\label{Example1}
Consider rolling a fair die and predicting whether the result is even or not. This can be described with the following probability model. Suppose $\Omega = \{1, \dots, 6\}$ and let $\F = 2^\Omega$ be all subsets of $\Omega$. The outcome is $Y = \one_A$, where $A = \{2, 4, 6\}$, and $\P$ is uniform over $\Omega$, that is, $\P(E) = |E|/6$ for all $E \in \F$.  Consider two forecasters with the information sets $\F_1 = \sigma(\{1\})$ and $\F_2 = \sigma(\{6\})$. In other words, the first forecaster knows whether the result is $1$ or not. The second forecaster, on the other hand, knows  whether the result is $6$ or not. Their combined information is   $\F'' = \sigma(\F_1, \F_2) =  \sigma(\{1\}, \{6\})$. 
%These information sets are illustrated in Figure \ref{ExamplePartitionFinite}. 
For each $\omega \in \Omega$, the relevant forecasts are
\begin{align*}
X_1(\omega) &= \begin{cases}
0 & \text{ if } \omega = 1\\
3/5& \text{ otherwise} 
...
\end{example}
This example constructs an outcome and a probability space under which the efficient aggregator $\mathcal{X}''$ is a random convex combination. This means that $\mathcal{X}_{[\cdot]}$ can be efficient. The example was chosen to illustrate that $\mathcal{X}_{[\cdot]}$ can do this if it is allowed to equal  the minimum or maximum forecast when some of the forecasts disagree. Fortunately, if Definition \ref{Xc} is tightened slightly such that this is not allowed, the class still contains essentially all measures of central tendency but general inefficiency results can be proven. 
...
\subsection{Random Strictly Convex Combinations}\label{StrictConvex}
...
Given that a measure of central tendency aims to summarize the forecasts with a single central value, the class of random strictly convex combinations contains essentially all of them. Surely, it is possible to construct simple examples where a measure of central tendency does not classify as a random strictly convex combination.  For instance, consider three forecasts that can only take on two different values. Every time some of the forecasts disagree, the median or the mode equals the minimum or maximum forecast. This is admittedly a rather contrived example, and for most complex and interesting cases, measures of central tendency can be assumed to fall within Definition \ref{Xsc}. 
...
Now, recall from Section \ref{outcomes} that $\F_j = \sigma(X_j)$ is generated by a random variable. This means that $\F_j$  is countably generated \citep{durrett2010probability} and  
...
% is countably generated. 
 hence is either finite or uncountably large (for this well-known result, see, e.g., \citealt{proschan2016essentials}). It is useful to analyze these cases separately. 
...
Therefore, to begin, suppose that the forecasters' information sets have finite cardinality. The next theorem gives a general inefficiency result under this constraint. 
...
Given that Definition \ref{Xsc} describes a highly general class of aggregators, detailed results akin to Theorems \ref{contraction} and \ref{contractionXphi}, can be hardly expected. Therefore it is important to emphasize that Theorem \ref{centralTendency} does not state that $\mathcal{X}_{(\cdot)}$ is under-confident or uncalibrated. Instead, it shows that $\mathcal{X}_{(\cdot)}$ never uses all the forecasters' information efficiently; that is, $\mathcal{X}_{(\cdot)}$ either distorts their information or leaves some of it unused. 

Suppose now that the forecasters' information sets are uncountably infinite. The next example shows that in such a case $\mathcal{X}_{(\cdot)}$ (and hence also $\mathcal{X}_{[\cdot]}$) can be efficient. Thus Theorem \ref{centralTendency} can not be relaxed to hold under any type of $\sigma$-fields.  
...
%\begin{minipage}[t]{.48\textwidth}
   ...
   \caption{The partitions $\tau_1$ and $\tau_2$ generate the forecasters' information. Together they form the partition $\tau''$ that generates their combined information. Under Lebesgue measure the forecasts are the middle points of each interval. Observe how $\mathcal{X}''$ is strictly between $X_1$ and $X_2$ under each $\omega \in [0,1]$.}
   \label{ExamplePartition}
%\end{minipage}
\end{figure}

\marginpar{Provide intuition for why this fails.}
...
Consider a probability space $(\Omega, \F, \P)$, where $\Omega = [0,1]$, $\F$ holds all the Borel subsets $\mathcal{B}([0,1])$, and $\P$ is the Lebesgue measure on $[0,1]$. The target outcome is defined as $Y(\omega) = \omega$ for all $\omega \in \Omega$. In other words, $Y$ is uniform over the unit interval. Consider $N = 2$ forecasters. To form their information, pick two strictly increasing countably infinite sequences $\tau_1 = \{a_k : k \in \mathbb{Z}\}$ and $\tau_2 = \{b_k : k \in \mathbb{Z}\}$
%\begin{align*}
%%\tau_1 &= \{\dots, a_{-3}}\} \\
...
%\end{align*}
where $\gamma_k = \sum_{j=0}^{k} \left( \frac{1}{2}\right)^j \to 2$ as $k \to \infty$. These sequences are illustrated in Figure \ref{ExamplePartition}. Any such sequences $\tau_1$ and $\tau_2$ define two countably infinite partitions of $\Omega$. The atoms of these partitions accumulate only at the extremes, and no atom in one partition is contained within an atom in the other partition.
%Given that $\gamma_k \to 2$ as $k \to \infty$, each collection forms a countably infinite partition of $\Omega$. 
The $j$th forecaster's information set $\F_j$ is then generated from the partition induced by $\tau_j$. This computes all possible disjoint unions of the atoms. The result is an uncountably infinite information set because it has the same cardinality as the power set of a countably infinite set. 
%The information set then has the same cardinality as the power set, which is uncountably infinite. 
 Their revealed information $\F''$ is generated by the partition
%\begin{align*}
...
\end{align}
Given the way $\tau_1$ and $\tau_2$ alternate, $\mathcal{X}''$ is always strictly between $X_1$ and $X_2$. Thus it is a random strictly convex combination. This shows that $\mathcal{X}_{(\cdot)}$ can be efficient if the forecasters' information sets are allowed to be uncountably large. Under the specific sequences (\ref{specificChoice}), the efficient aggregator (\ref{Example2Rev}) becomes
\begin{align*}
\mathcal{X}''
%&= \begin{cases}
...
Given that the inefficiency result (Theorem \ref{centralTendency}) does not hold under uncountably infinite $\sigma$-fields, it is natural to ask whether such $\sigma$-fields exist in the real world. 
...
Therefore, even though a forecaster with infinite information may be a convenient approximation in some theoretical work, it is not a precise description of reality. This gives us the following corollary that concludes our analysis of measures of central tendency.
\begin{corollary}[Inefficiency of Measures of Central Tendency] \label{MCTCor}
For all practical purposes, measures of central tendency do not use the forecasters' information efficiently. 
\end{corollary}
...
%\subsection{Noisy Forecasts} \label{noisy}
Real-world forecasts are unlikely to be perfectly calibrated. Instead, the forecasters are likely to make some mistakes in interpreting their information and turning that into an over prediction. This can be taken into account by adding an error term to the calibrated forecast. More specifically, if $\tilde{X}_j$ is the $j$th forecaster's noisy prediction, then $\tilde{X}_j = X_j + \epsilon_j$, where $\epsilon_j \sim \mathcal{N}(0,\sigma^2)$ is some idiosyncratic error. Given that the average is a consistent estimator of the mean, the error terms can be averaged out, leading to
\begin{align*}
\frac{1}{N} \sum_{j=1}^N \tilde{X}_j \approx \frac{1}{N} \sum_{j=1}^N X_j
\end{align*}
for a sufficiently large $N$. Intuitively, a similar observation holds for other measures of central tendency as well because the symmetric error $\epsilon_j$ spreads the forecasts equally relative to the center. Therefore, given that adding noise will not change the aggregate dramatically, results in this article also provide insight about aggregating uncalibrated predictions. 
...
%\subsection{Efficient Aggregation in Practice} \label{practiceEfficient}
If all measures of central tendency are inefficient, a natural question to ask is whether an efficient aggregator even exists. Fortunately, the answer is ``yes.''  In fact, there are at least two different approaches to constructing such aggregators.
% if $\boldsymbol{X} = (X_1, \dots, X_N)$ and $\boldsymbol{\beta}$ is some vector of unknown parameters,
The first approach supposes that $\mathcal{X}''$ has a particular functional form. For instance, if the forecasts are calibrated and $\mathcal{X}''$ is a linear function of the forecasts, then by minimizing the expected quadratic loss it is possible to show that $\mathcal{X}'' = \mu_0 + \diag(\bSigma)'\bSigma^{-1}(\boldsymbol{X}-\mu_0 \one_N)$, where $\boldsymbol{X} = (X_1, \dots, X_N)$ and $\bSigma := \Cov(\X,\X)$ satisfies $\delta_0 -  \diag(\bSigma)'\bSigma^{-1}\diag(\bSigma) \geq 0$. See \cite{satopaamodeling} for more details on the derivation and non-parametric estimation of $\bSigma$.
...
a generic model such as the \textit{Gaussian partial information model} that  \citet{satopaamodeling2} introduce as a default choice for forecast aggregation. This is a general yet very flexible model that can be modified and simplified to meet the needs of a broad range of forecasting applications. For instance, it has been applied to forecasters making  predictions of multiple related events \citep{satopaamodeling2} and also to only two forecasters, each making a single probability prediction \citep{ernst2016bayesian}. 
...
%holds for the transformed averages after adding $\epsilon_j$ to the transformed forecasts $\Phi(X_j)$. 
...
%  the probability forecasts; \cite{satopaa} use a logistic regression model to extremize the average log-odds of the
%  forecasts; many others, including  \cite{shlomi2010subjective}, \cite{baron2014two}, and  \cite{mellers2014psychological}, have also discussed extremization of probability forecasts. 
Theorem \ref{contraction} suggests that extremization can be beneficial even when the outcome is not binary. This may seem somewhat surprising because real-valued outcomes are  not at the border of the range of possible forecast values.  
...
\marginpar{For one, the Wisdom of the Crowds argues that if the crowd were a single person, that was how much it would have guessed the ox weighted. This refers to the mean. page 9. This is not too off actually because they all have the same. This is where the mean works. OK. Let it be.}
...
\item Recall that $\mathcal{X}''$ is always calibrated. Thus, by item ii), $\mathcal{X}_w$ is not efficient if it assigns positive weight to at least two different forecasts.
...
Let $\X = (X_1, \dots, X_N)'$. If $\Cov(\X,\X) := \bSigma$,  then it is well-known that $\argmin_{\bBeta} \E(Y- \bBeta\X)^2 =  \Cov(\X, Y) \bSigma^{-1} =: \bBeta^*$ , where $\Cov(\X, Y) = (\Cov(X_1,Y), \dots, \Cov(X_N,Y))$. Now, \cite{satopaamodeling2} show that if $X_j$ is calibrated, then $\Cov(X_j, Y) = \Var(X_j)$. Therefore $\bBeta^* = \diag(\bSigma)\bSigma^{-1}$. If $\Cov(X_j,X_i) = 0$ for all $i \neq j$, then $\bSigma = \diag(\bSigma)$, $\bBeta^* = \one_N'$, and $\bBeta^* \X = \sum_{j=1}^N X_j$. Therefore independent information combines via direct summing. 

Consider now the case of $N=2$ forecasters. Then, $\bBeta^* = \diag(\bSigma)\bSigma^{-1} = (\delta_1\delta_2 -\rho\delta_2, \delta_2\delta_2 -\rho\delta_1) / (\delta_1\delta_2 - \rho^2)$ and $\beta_1 + \beta_2 = (2\delta_1\delta_2 - \rho\delta_1 - \rho\delta_2) / (\delta_1\delta_2 - \rho^2)$. Given that
\begin{align*}
\frac{\partial}{\partial \rho} (\beta_1 + \beta_2) &= - \frac{\delta_2(\delta_1-\rho)^2 + \delta_1(\delta_2-\rho)^2}{(\rho^2 - \delta_1\delta_2)^2} \leq 0,
\end{align*}
the sum $\beta_1 + \beta_2$ is a monotonically decreasing function of $\rho$. Note that $\delta_ j = \Var(X_{(j)}) + \Var(X_{(1,2)}) \geq \Var(X_{(1,2)}) = \rho$. Thus $\rho \leq \min(\delta_1,\delta_2)$. Suppose without loss of generality that $\delta_1 \leq \delta_2$. Then, the minimum of $\beta_1 + \beta_2$ occurs when $\rho = \delta_1$. This minimum is
\begin{align*}
\min (\beta_1 + \beta_2) &= (\beta_1 + \beta_2) |_{\rho = \delta_1} = \frac{2\delta_1\delta_2 - \delta_1^2 - \delta_1\delta_2}{\delta_1\delta_2 - \delta_1^2} = 1.
\end{align*}
Finally, to show that $\mathcal{X}''$ is the orthogonal projection of $\mathcal{X}'$ onto the space of all aggregators of the form $\beta_1 X_1 + \beta_2 X_2$, refer to the well-known result mentioned in the beginning of this section and get $\argmin_{\bBeta} \E(\mathcal{X}'- \bBeta\X)^2 =  \Cov(\X, \mathcal{X}') \bSigma^{-1}$. Now, $\Cov(X_j, \mathcal{X}') = \Cov(X_{(j)} + X_{(1,2)}, X_{(j)} + X_{(i)} + X_{(1,2)}) = \Cov(X_{(j)}, X_{(j)}) + \Cov(X_{(1,2)},X_{(1,2)}) = \Var(X_j)$. Thus $ \Cov(\X, \mathcal{X}') \bSigma^{-1} = \diag(\bSigma)\bSigma^{-1}$ as desired.
...
%Now, recall from Section \ref{InfoCollection} that all calibrated forecasts are marginally consistent. Given that $\E(\mathcal{X}_\Phi) \neq \mu_0$ when $\Phi$ is strictly convex or concave, it is not calibrated either. 
...
%&= \sum_{j=1}^N w_j X_j(\omega)
  \mathcal{X}_w (\omega),
\end{align*}
where the equality holds if and only if $\Phi(X_1(\omega)) = \dots = \Phi(X_N(\omega))$ or $\Phi$ is linear. Given that $\mathcal{X}_\Phi$ assigns weight to two different forecasts, the event $A = \{\omega : \exists i \neq j \text{ s.t. } X_j(\omega) \neq X_i(\omega)\}$ has positive probability $\P(A)  > 0$. Thus $\E(\mathcal{X}_\Phi) < \E(\mathcal{X}_w)$ unless, conditional on $A$, $\Phi$ is a.s. linear around the forecasts. But if this is the case, then $\mathcal{X}_\Phi(\omega) = \mathcal{X}_w(\omega)$ a.s., which, by Theorem \ref{contraction}, shows that $\mathcal{X}_\Phi$ is not calibrated.  If, conditional on $A$, $\Phi$ is not a.s. linear around the forecasts, then $\E(\mathcal{X}_\Phi) < \E(\mathcal{X}_w)$. Given that calibration implies marginal consistency, $\mathcal{X}_\Phi$ is not calibrated in this case either. The case of a concave $\Phi^{-1}$ follows similarly. 
...
Proof by contradiction:  Suppose the efficient aggregator $\mathcal{X}''$ is a random strict convex combination. Consider a probability space $(\Omega, \F, \P)$. Given that each $\F_j$ is finite, there exists a unique finite partition $\tau_j = \{A_{1j}, A_{2j}, \dots, A_{n_jj}\}$ of $\Omega$ such that $\F_j = \sigma(\tau_j)$. The corresponding forecast $X_j$ is constant within any atom $A \in \tau_j$. Denote this constant forecast with $X_j(A)$. The forecasters' combined information is $\F'' = \sigma\left( \cup_{j=1}^N \F_j \right) =\sigma(\tau'')$, where $\tau'' := \{C_1, \dots, C_{n''}\} =  \{C : C \neq \emptyset, C = \cap_{j=1}^N A_{j}, A_{j} \in \tau_j\}$ is a generating partition of $\Omega$. This partition is finite $|\tau''| < \infty$ because each $|\tau_j| < \infty$. 
...
Given that $\tau''$ is a finer partition than any $\tau_j$, the forecasts and $\mathcal{X}''$  are constant within any atom $C  \in \tau''$. Similarly to before, denote these forecasts with $\mathcal{X}''(C)$ and $X_j(C)$ for $j = 1, \dots, N$. By construction, each atom $A_{ij} \in \tau_j$ can be written as a unique union of some atoms in $\tau''$.  Given that $X_j = \E(Y|\F_j)$, for any atom $A_{ij} \in \tau_j$ such that $\P(A_{ij}) > 0$,
\begin{align}
 \int_{A_{ij}} Y d\P = \int_{A_{ij}} X_j d\P &= X_j(A_{ij}) \int_{A_{ij}}  d\P  = X_j(A_{ij}) \P(A_{ij}) \nonumber\\ 
\Leftrightarrow X_j(A_{ij}) = \frac{\int_{A_{ij}} Y d\P}{\P(A_{ij})} &= \frac{\sum_{k : C_k \cap A_{ij} \neq \emptyset}\int_{C_k} Y d\P}{\P(A_{ij})} \nonumber \\
 &= \frac{\sum_{k : C_k \cap A_{ij} \neq \emptyset}\mathcal{X}''(C_k) \P(C_k) }{\P(A_{ij})}\nonumber\\
 &= \sum_{k : C_k \cap A_{ij} \neq \emptyset}p_{ijk}\mathcal{X}''(C_k), \label{decomposition}
\end{align}
where $p_{ijk} = \P(C_k) / \P(A_{ij})$ and $\sum_{k : C_k \cap A_{ij} \neq \emptyset}p_{ijk} = 1$. Therefore every value of $X_j$ can be written as a weighted average of some values of $\mathcal{X}''$. 
...
Now, pick some set $C \in \tau''$ with $\P(C) > 0$ and suppose that the forecasts $X_1(C), \dots, X_N(C)$ do not all agree on this set. Define two indices 
\begin{align*}
{k}^* &:= \argmin_j\{X_j(C)\} && \text{ and} &
k^{**} &:= \argmax_j\{X_j(C)\}.
\end{align*}
...
Given that the forecasts do not all agree, $\mathcal{X}''$ is in the interior of their convex hull:
% for a $C_k$ such that $\P(C_k) > 0$, 
% Similarly define $q_{jk} = \sum_{w \in C_k} \P(w)  / \sum_{w \in B_j} \P(w)$. 
% and hence would contradict our assumption.
%\scriptsize
\begin{align*}
X_{k^*}(C) &< \mathcal{X}''(C) < X_{k^{**}}(C)\\
\Leftrightarrow \sum_{k : C_{k} \cap A_{ik^*} \neq \emptyset, C \cap A_{ik^*} \neq \emptyset}p_{ik^*k}\mathcal{X}''(C_{k}) &< \mathcal{X}''(C)  <    \sum_{k : C_{k} \cap A_{ik^{**}} \neq \emptyset, C \cap A_{ik^{**}} \neq \emptyset}p_{ik^{**}k}\mathcal{X}''(C_{k}).
%\frac{\sum_{w \in  A_1 / C_{11}} \P(w) Y(w) + \sum_{w \in C_{11}} \P(w) Y(w)}{\sum_{w \in A_1} \P(w)} &< \frac{\sum_{w \in C_{11}} \P(w) Y(w)}{\sum_{w \in C_{11}} \P(w)}  < \frac{\sum_{w \in B_1 / C_{11}} \P(w) Y(w) + \sum_{w \in C_{11}} \P(w) Y(w)}{\sum_{w \in B_1} \P(w)}\\
...
\end{align*}
The second step uses (\ref{decomposition}) and bounds $\mathcal{X}''(C)$ strictly from below and above by a convex combination of some $\mathcal{X}''(C_{k})$s. Both sums include $\mathcal{X}''(C)$.  In order for the strict inequalities to hold, however, the weight $p_{ijk}$ associated with $\mathcal{X}''(C)$ in these sums cannot be $1.0$. Therefore $\mathcal{X}''(C)$ and at least one other $\mathcal{X}''(C_k)$ in each sum enter the convex combination with a positive weight. Among these elements $\mathcal{X}''(C)$ cannot be the smallest on the left (or the largest on the right) because there must be some element that is strictly smaller (or larger), has positive weight, and hence pulls the convex combination away from $\mathcal{X}''(C)$. Thus, there exists indices $k'  \neq k''$ such that
\begin{align}
\mathcal{X}''(C_{k'}) &< \mathcal{X}''(C)  < \mathcal{X}''(C_{k''}), \label{strictIE}
\end{align}
%where both
%\begin{align*}
...
%are well-defined. This shows that each $\mathcal{X}''(C_k)$ is upper and lower bounded by $\mathcal{X}''(C_{k'})$ and $\mathcal{X}''(C_{k''})$
 where $\P(C_{k'}) > 0$, $\P(C_{k''}) > 0$, $C_{k'} \neq C$, and $C_{k''} \neq C$. Thus, if the forecasts do not all agree on some $C$, then $\mathcal{X}''(C)$ is strictly upper and lower bounded by some of its other values.  Furthermore, these values are linked to $C$ via some $A_{ij} \in \tau_j$, that is, there are some $A_{ik^*}$ and $A_{ik^{**}}$ such that $C \cup C_{k'} \subseteq A_{ik^*}$ and $C \cup C_{k''} \subseteq A_{ik^{**}}$. 
 
 
 
 Given that $\E(|Y|) < \infty$, we have that $\E(|\mathcal{X}''|) < \infty$. In other words, $|\mathcal{X}''| < \infty$ on all $C \in \tau''$ with $\P(C) > 0$. Now, let the set $\mathcal{C}_1 = \{C : \mathcal{X}''(C) = \min(\mathcal{X}''(C)), C \in \tau''\}$ collect all atoms of $\tau''$ on which $\mathcal{X}''$ reaches its minimum. If some of the forecasts disagree on any of the atoms in $\mathcal{C}_1$, (\ref{strictIE}) shows that some value of $\mathcal{X}''$ must be strictly smaller than its minimum. This is a contradiction. Thus assume that the forecasts agree on the atoms in $\mathcal{C}_1$. Given that $\mathcal{X}''$ is a random strict convex combination, it is equal to this unanimous forecast. This kind of complete agreement among forecasts is possible only if the decomposition (\ref{decomposition}) for all forecasts consists only of $\mathcal{X}''(C)$ with $C \in \mathcal{C}_1$; including any other value of $\mathcal{X}''$ would increase the forecast and hence make it larger than  $\mathcal{X}''$. 
% This, of course, is not possible because on this set all forecasts equal $\mathcal{X}''$. 
% Thus if $C \subseteq A_{ij}$ for some $C \in \mathcal{C}$, then $A_{ij} = \cup_k C_k$ for some $C_k \in \mathcal{C}$. In other words, 
 Thus $\mathcal{C}_1$ is a disjoint set in a sense that $C \in \mathcal{C}_1$ and $C' \notin  \mathcal{C}_1$ cannot both be contained in some $A_{ij}$. Next, let the set $\mathcal{C}_2 = \{C : \mathcal{X}''(C) = \min(\mathcal{X}''(C)), C \in \tau'' \setminus \mathcal{C}_1\}$ collect all atoms of $\tau''$ on which $\mathcal{X}''$ reaches its second smallest value. Analyze $\mathcal{C}_2$ similarly to the above analysis of $\mathcal{C}_1$. In particular, if the forecasts disagree on some $C \in \mathcal{C}_2$, (\ref{strictIE}) leads to a contradiction because there is no smaller value of $\mathcal{X}''$ that can be reached via some $A_{ij}$. If, on the other hand, all forecasts agree, $\mathcal{C}_2$ is a disjoint set just like $\mathcal{C}_1$. 
% The analysis then proceeds to the analysis of the set $\mathcal{C}_3 = \{C : \mathcal{X}''(C) = \min(\mathcal{X}''(C)), C \in \tau \setminus (\mathcal{C}_1 \cup \mathcal{C}_2)\}$.
  This process can be repeated until one encounters a set $\mathcal{C}_k$ with an atom on which some forecasters disagree. Such a set must exist because at least two forecasters are assumed to disagree with positive probability.  This then gives the contradiction as argued before. 
