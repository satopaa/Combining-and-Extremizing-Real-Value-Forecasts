...
Denote the future outcome of interest with $Y$. Suppose $Y$ is a random variable with a finite mean $\E(Y) := \mu_0 < \infty$ and a finite variance $\Var(Y) := \delta_0 < \infty$ under some
 probability space
$(\Omega, \F , \P)$. Intuitively, this space is a probabilistic model for the way $Y$ behaves in practice. The main results in this paper do not rely on a specific instance of $(\Omega, \F , \P)$ but instead apply to all probability models jointly. The only assumption is the existence of such a space. This, however, cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].''  
%The set $\Omega \neq \emptyset$ represents all possible states of nature and $\F = \mathcal{P}(\Omega) = \{A : A \subset \Omega\}$ is the set of all subset of $\Omega$. 
...
This question is explored more rigorous analysis in the following sections. 
...
\begin{corollary}
$\mathcal{X}_w$ is inefficient as long as it assigns positive weight to at least two different forecasts. 
\end{corollary}
...
This section expands Definition \ref{Xw} to allow transformations of the forecasts before they are averaged. Typically the goal is to transform the forecasts $X_j$ to some other space where they are more symmetric around their mean \citep{baron2014two}. The following definition makes this class of aggregators precise.
\begin{class}[Transformed Weighted Averages] \label{Xphi}
Consider some strictly monotone function $\Phi$. The transformed weighted average associated with $\Phi$ is $\mathcal{X}_{\Phi} := \Phi^{-1} \left( \sum_{j=1}^N w_j \Phi(X_j) \right)$, where each weight $w_j \geq 0$ is fixed and $\sum_{j=1}^N w_j = 1$. 
\end{class} 
The function $\Phi$ is strictly monotone so that its inverse exists and two different forecasts are not mapped to the same value. From an information perspective, an invertible function preserves information; that is, $\sigma(X_j) = \sigma(\Phi(X_j))$. Clearly, if $\mathcal{X}_\Phi$ is to be efficient, the transformation $\Phi$ cannot lose any of the original information content. Thus, invertibility is essential.  Despite this requirement, however, the following theorem shows that many transformed weighted averages are inefficient. 
...
Suppose $\mathcal{X}_\Phi$ assigns positive weight to at least two different forecasts. Then the following holds.\begin{enumerate}[i)]
\item If $\Phi$ is convex or concave, then $\mathcal{X}_\Phi$ is neither calibrated nor efficient. Furthermore, if $\Phi$ is strictly convex or concave, then  $\mathcal{X}_\Phi$ is not even marginally consistent.\label{convex} 
\item If  $X_j \geq 0$ and $\Phi$ is continuous, then $\mathcal{X}_\Phi$ is inefficient. \label{mono}
\end{enumerate}
\end{theorem}
...
\end{align*}
where each $X_j \geq 0$ and $a \in \R$ is a given constant. Here the function $\Phi(x) = x^{a}$ is concave if $a \in [0,1]$ but convex otherwise. Notable members of this class include the harmonic, geometric, arithmetic, quadratic, and cubic means, corresponding respectively to $a = -1, 0, 1, 2, 3$. Thus, by item \ref{convex}) of Theorem  \ref{contractionXphi}, none of these aggregators is efficient as long as they assign positive weight to at least two different forecasts. In fact, they are not even calibrated and hence based on any information set about $Y$.

\marginpar{Maybe show here why it is convex etc.}
...
Suppose now that the forecasters' information sets are uncountably infinite. In such a case, $\mathcal{X}_{(\cdot)}$ can be efficient and, consequently, the conditions of Theorem \ref{centralTendency} can not be relaxed to any type of $\sigma$-fields.  This is illustrated in the next example. 
...
%&= \mathcal{X}_{\Phi_+},
\end{align*}
where, by construction, $\Phi(X_j) - M_{\epsilon}  > 0$ for all $j = 1, \dots, N$. This shows that the weighted average can be taken anywhere on the real line as long as one remembers to shift it back to the original center. This is important because it makes the term associated with $w_j$ positive. 
...
Overall, our goal is to compute $\frac{\partial }{\partial w_j} \E\left(\mathcal{X}_\Phi^2\right)$ for all $j = 1, \dots, N$ and then find the weights $\w \in \Delta_N$ that maximize $\E\left(\mathcal{X}_\Phi^2\right)$. This requires us to justify switching the order of integration (expectation) and differentiation within $\Delta_N$. Given that
...
\begin{align*}
\E[|\mathcal{X}_\Phi^2|]  = \E[\mathcal{X}_\Phi^2]  &\leq  \E[\max\{X_j\}^2]\\
&<  \sum_{j=1}^N \E[X_j^2]\\
&=  \sum_{j=1}^N \Var[X_j] + \E[X_j]^2\\
&=  \sum_{j=1}^N \delta_j + \mu_0^2 \leq  N (\delta_0 + \mu_0^2) < \infty,
\end{align*}
$\mathcal{X}_\Phi^2$ is Lebesgue-integrable under each $\w$. Furthermore, for almost all $\omega \in \Omega$, the partial derivative  $\frac{\partial }{\partial w_j} \mathcal{X}_\Phi^2$ must exist for all $w_j$. This derivative is
\begin{align*}
 \frac{\partial }{\partial w_j}  \mathcal{X}_\Phi^2
...
%&> 0
\end{align*}
First, given that $\frac{\partial }{\partial x}\Phi^{-1} \left( x \right) > 0$ (from strict monotonicity), $\mathcal{X}_\Phi \geq 0$ (from theorem hypothesis), and $\Phi(X_j) - M_\epsilon > 0$ (from beginning of the proof), this partial derivative is always non-negative. Second, given that $X_j$ is integrable, $X_j$ is almost everywhere finite. The aggregate $\mathcal{X}_\Phi$ is constrained within the convex hull of the forecasts and therefore also almost everywhere finite. Given that $\Phi$ is differentiable and  $\frac{\partial}{\partial x}\Phi(x) > 0$,  the inverse function $\Phi^{-1}$ is also differentiable \citep{lay2015analysis}. Putting this all togethers shows that the partial derivative  $\frac{\partial }{\partial w_j} \mathcal{X}_\Phi^2$ exists. 

Given that $\Phi$ is continuously differentiable and strictly monotone, the Inverse Function Theorem tells us that the inverse function $\Phi^{-1}$ is also continuously differentiable. Thus $\Phi^{-1}$ is locally Lipschitz. 

%The final step is to justify bring the partial derivative inside the expectation. To do this, let \marginpar{This justification needs a bit more work; transforming a finite Xj cannot blow up.}
%\begin{align*}
%u(t) &= \Phi^{-1} \left(  t(\Phi(X_j)-M_\epsilon)+ \sum_{k \neq j} w_k (\Phi(X_k)-M_\epsilon)   + M_\epsilon \right)^2 
%\end{align*}
%By the Fundamental Theorem of Calculus, the derivative is 
%\begin{align*}
%\frac{\partial }{\partial w_j} \E[\mathcal{X}_\Phi^2] &= \lim_{t \uparrow w_j} \frac{\E[u(w_j)] - \E[u(t)]}{w_j - t} =  \lim_{t \uparrow w_j} \E \left[ \frac{u(w_j) - u(t)}{w_j - t}\right]
%%&= \lim_{t \uparrow w_j}   \E \left[ \frac{\mathcal{X}_\Phi^2 - \Phi^{-1} \left(  t\Phi(X_j) + \sum_{k \neq j} w_k \Phi(X_k)  \right)^2 }{w_j - t}\right]
%\end{align*}
%Plug $t = w_j - 1/n$ into the integrand and let 
%\begin{align*}
%f_n := \frac{u(w_j) - u(w_j - 1/n)}{1/n}
%\end{align*}
%Given that $\Phi^{-1}$ is monotonically increasing and $\Phi(X_j)-M_\epsilon > 0$, $u(t)$ is monotonically increasing in $t$. This means that $u(w_j) \geq u(w_j - 1/n)$ for all $n \in \mathbb{N}$. Then, $0 \leq f_n \uparrow f = \frac{\partial }{\partial w_j} \mathcal{X}_\Phi^2$. Therefore by the Monotone Convergence Theorem
%\begin{align*}
%\frac{\partial }{\partial w_j} \E[\mathcal{X}_\Phi^2] = \lim_{n \to \infty} \E[f_n] =  \E[f] =  \E \left[ \frac{\partial }{\partial w_j} \mathcal{X}_\Phi^2\right]
%\end{align*}


\marginpar{Not sure if this works: how we do know fn is increasing in n}

%\begin{enumerate}
%\item \textbf{Condition 2:}  For almost all $\omega \in \Omega$, $\frac{\partial}{\partial w_j}\mathcal{X}_\Phi^2$ exists for all $w_j$ within the $N$-simplex.\\
%\textbf{Proof.} Given that $\Phi$ is strictly monotonic, its inverse $\Phi^{-1}$ is also monotonic. Recall that the composition of monotonic functions is monotonic. Therefore $\mathcal{X}_\Phi^2$ is a monotonic function of $w_j$. 
%
%
%The derivative is 
%\begin{align*}
...
%%&= \E\left[ \frac{\partial }{\partial w_j}  \Phi^{-1} \left( \sum_{j=1}^N w_j \Phi(X_j ) \right)^2 \right] \\
%%&= 2 \Phi^{-1} \left( \sum_{j=1}^N w_j \Phi(X_j) \right) (\Phi(X_j)- \min \{\Phi(X_j)\} + \epsilon)  \frac{\partial }{\partial x}\Phi^{-1} \left( x \right) \bigg|_{\sum_{j=1}^N w_j \Phi(X_j )}  \\ 
%&= 2 \mathcal{X}_\Phi (\Phi(X_j)- \min \{\Phi(X_j)\} + \epsilon)  \frac{\partial }{\partial x}\Phi^{-1} \left( x \right) \bigg|_{\sum_{j=1}^N w_j \Phi(X_j )} 
%%&> 0
%\end{align*}
%Each $X_j$ is integrable and hence almost surely finite. Given that $\mathcal{X}_\Phi$ is within the convex hull of $X_j$, it is also almost surely finite. We must assume that $\Phi$ is differentiable on the appropriate interval. Given that $\Phi' >\neq 0$, by the inverse function theorem, $\Phi^{-1}$ is also differentiable. 
%
% 
%\item \textbf{Condition 3:}  There is an integrable function $\theta: \Omega \to \R$ such that $|\frac{\partial}{\partial w_j}\mathcal{X}_\Phi^2| \leq \theta(\omega)$ for all $w_j$ and almost all $\omega \in \Omega$. \\
%\textbf{Proof.} 
%\begin{align*}
%\frac{\partial }{\partial w_j}  \mathcal{X}_\Phi^2
%%&= \E\left[ \frac{\partial }{\partial w_j}  \Phi^{-1} \left( \sum_{j=1}^N w_j \Phi(X_j ) \right)^2 \right] \\
...
%\end{align*}
%\end{enumerate}
Now, changing the other of differentiation and integration is justified. This gives us
\begin{align}
