...
Suppose now that there are $N \in \mathbb{N}$ forecasters predicting $Y$. Denote the $j$th forecaster's prediction with the random variable $X_j \in \F$, $j = 1, \dots, N$. Akin to the previous studies discussed in Section \ref{prevWork},
...
Calibration turns out to be precisely the minimal condition that the forecasters must satisfy in order to fulfill their responsibility in the two-step collective forecasting procedure. More specifically, 
...
Therefore the are many practical ways that the forecasters can be guided to fulfill their responsibility of reporting only truthful and relevant information about the outcome. This information is then given to the aggregator whose responsibility is to combine it efficiently.
...
They are defined as any forecasts that are measurable with respect to $\F'' := \sigma(X_1, \dots, X_N)$, namely the $\sigma$-field revealed by the forecasts or, equivalently, information generated by the forecasters' information sets $\F_j$, $j = 1, \dots, N$.  Figure \ref{CollectInfo} gives a simple illustration of the relationship between $\F$, $\F''$, and $\F_j$ for $N = 3$ forecasters. An aggregator is considered efficient if there exists some probability model under which it uses all of $\F''$ to construct a consensus forecast. This is  made precise in the following definition.
...
\subsection{Weighted Averaging} \label{contractionSection}
...
\marginpar{Explain why it distorts information}
...
In particular, if the total weight is represented with a pie, public and private information receive slices that are proportionally the same size but $\mathcal{X}''$ can change the size of its pie.
% is cutting a pie that is $2c$ times larger than that of $\bar{\mathcal{X}}$.


\marginpar{Place the derivations into the appendix: the coefficient and range}
...
\subsection{Transformed Weighted Averaging} \label{TranscontractionSection}
...
\subsubsection{Random Convex Combinations}
...
\subsubsection{Random Strictly Convex Combinations}
