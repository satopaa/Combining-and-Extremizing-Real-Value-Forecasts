...
provide quarterly predictions of gross domestic product (GDP). These
forecasts are then combined into a single consensus forecast both for
the sake of decision-making and
accuracy \citep{armstrong2}. The aggregation can be
performed in many different ways, and the final combination rule can
significantly affect the predictive accuracy of the consensus
forecast. The paper presents method of forecast
aggregation that aims to combine information from multiple forecasts
into a single forecast with optimal properties.

Weighted averaging, a.k.a. linear opinion pooling, is by far the most
popular method for combining forecasts. 
Averaging forecasts has a long tradition (Bates and
Granger,1969; Clemen, 1989; Armstrong, 2001), and 
although averages do not always beat the best single forecaster (Hibon and Evgeniou) -- one does
not in general know ex ante which expert will be the most accurate, so ,
forecast averaging is still considered state-of-the-art ( Graham Elliott and Allan Timmerman).
...
forecasts, included real-valued forecasts. Our result shows, for instance, that any non-trivial
...
information framework \citep{satopaamodeling2, satopaamodeling}  and
...
\textit{Extremizing}, transforming the weighted average forecast away from the marginal mean,
in being increasingly used in the probability forecasting literature as a means of
alleviating systematic under-confidence{\cite{Ranjan08,satopaa,baron2014two}.
%For instance, \cite{Ranjan08} propose a beta transformation that extremizes the weighted average of
%the probability forecasts; \cite{satopaa} use a logistic regression
%model to extremize the average log-odds of the
%forecasts; \cite{baron2014two} propose another extremizing
%transformation and give a psychological reason why human forecasters' probability estimates tend to be under-confident. 
In this paper we suggest that real-valued forecasts should also be extremized -- pushed away from {\bf what does extremizing mean in the real-valued setting?}:
