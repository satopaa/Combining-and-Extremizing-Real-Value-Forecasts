...
%There are, of course, many different ways one could combine predictions.
%In general, forecasts can be combined in many different ways.
Even though it is unclear which option should be preferred in general, the forecasting literature by and large agrees that aggregation of forecasts is beneficial \citep{clemen, armstrong2001combining}. By far the most common aggregator is the simple average. Its popularity is largely due to its familiarity, simplicity, and good past performance in 
% Panetta could combine the predictions in many different ways. Most people would probably first consider the simple average. 
%has performed well
 a variety of settings \citep{makridakis1983averages,clemen1986combining,clemen1989combining}.
% As long as the forecasts bracket the true outcome, that is, the outcome falls within their convex hull, averaging
...
The natural question then to ask is whether an efficient aggregator even exists. Fortunately, the answer is ``yes.'' Even though such construction are not the main focus of this paper, Section \ref{practiceEfficient}  briefly describes two general recipes for deriving efficient aggregators in practice. 
...
Suppose there are $N \in \mathbb{N}$ forecasters predicting the value of some future outcome. Denote this outcome with $Y$ and its $N$ information-driven point forecasts with $X_1, \dots, X_N$. \cite{satopaamodeling2, satopaamodeling} propose the \textit{partial information framework} as a general platform for analyzing and interpreting such setups. This framework assumes the outcome to behave as follows:
%This framework operates under  
%%, that forms the basis for our analysis of efficient forecast aggregation.
%%To begin, consider 
%some probability model $(\Omega, \F , \P)$. Our first assumption is about the outcome:
\begin{customthm}{1}
...
% probability space
%$(\Omega, \F , \P)$.
Denote the mean and variance of $Y$ with $\E(Y) := \mu_0 < \infty$ and $\Var(Y) := \delta_0 < \infty$.
Even though $Y$ is defined on a probability space, our main results do not depend on any specific choice but instead apply to all such spaces jointly. The only assumption is the existence of such a space. This, however, cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].''  
...
% The aggregator only operates on the reported forecasts and hence can access only
  information these forecasts reveal. Therefore, for the sake of aggregation, any unused information may as well not exist, and it is in no way restrictive to set $\F_j = \sigma(X_j)$. This allows us to express the $j$th forecast as $X_j = \E(Y | X_j)$.
  
   In the statistical and meteorological forecasting literature this property is known as \textit{calibration} (see, e.g., \citealt{dawid1995coherent, Ranjan08, jolliffe2012forecast}).  Given that it depends on the probability measure $\P$, it is somewhat different from the widely studied notion of empirical calibration
%This dependency is the main conceptual difference between $\P$-calibration and empirical calibration 
(\citealt{dawid1982well, foster1998asymptotic}; and many others). 
%Empirical calibration considers a stream of realized outcome-forecast pairs $(y_k, x_k)$, for $k \in \mathbb{N}$. The forecaster is empirically calibrated if the average outcome equals $x$ among all pairs for which $x_k = x$; and this is true for all possible values of $x$. 
...
%Calibration is the minimal condition that the forecasters must satisfy in order to fulfill their responsibility in collective forecasting. In fact,  a forecast $X$ is calibrated if and only if it is based on some partial information set, that is, $X = \E(Y|\G)$ for some $\G \subseteq \F$ \citep{satopaamodeling}. 
Assuming calibrated forecasts is convenient because, instead of somehow trying to identify true information and turning that into a prediction, the forecasters can now fulfill their responsibility in collective forecasting by focusing on the more concrete goal of being calibrated.
...
\marginpar{Pull from Zhi's paper quote on "not only for people but also for models" In fact models tend to be more calibrated but no need to say that.}
...
an increasing sequence of $\sigma$-fields $\F_0 = \{\emptyset, \Omega\} \subseteq \F_1 \subseteq \dots \subseteq \F_R \subseteq \F$ and the corresponding forecasts $X_r = \E(Y|\F_r)$ for $r = 0,1, \dots, R$. By \citealt[Proposition 2.1.]{satopaamodeling2}, the variances of these forecasts follow the same order as their nested information sets, that is, $\Var(X_0) = 0 \leq \Var(X_1) \leq \dots \leq \Var(X_R) \leq \Var(Y) = \delta_0$ \citep{}. Thus the amount of information in a calibrated forecast is measured by its variance. Naturally, if an aggregator is efficient, it uses all of the forecasters' information and hence uses at least as much information as the most informed individual among them. Therefore the efficient aggregator $\mathcal{X}''$ is necessarily at least as variable as each of the individual forecasts. This is precisely what item iii) describes. 
...
If all measures of central tendency are inefficient, the natural question to ask is whether an efficient aggregator even exists. Fortunately, the answer is ``yes.''  In fact, there are at least two different approaches to constructing such aggregators. More specifically, if $\boldsymbol{X} = (X_1, \dots, X_N)$ and $\boldsymbol{\beta}$ is some vector of unknown parameters, then the first approach supposes that $\E(Y|\boldsymbol{X} ) = \gamma(\boldsymbol{X};\boldsymbol{\beta})$ for some function $\gamma$ with a known dependency on $\boldsymbol{\beta}$. The value of $\boldsymbol{\beta}$ is then determined by minimizing the expected Bregman loss.  For instance, if $\gamma(\cdot)$ is a linear function of the forecasts, then $\boldsymbol{\beta} = (\mu_0, \bSigma)$ and $ \gamma(\boldsymbol{X}; \mu_0, \bSigma) = \mu_0 + \diag(\bSigma)'\bSigma^{-1}(\boldsymbol{X}-\mu_0 \one_N)$, where  and $\bSigma := \Cov(\X,\X)$ satisfies $\delta_0 -  \diag(\bSigma)'\bSigma^{-1}\diag(\bSigma) \geq 0$. For more details on the derivation and how to estimate $\bSigma$ in practice, see \cite{satopaamodeling}.
...
The second approach assumes a parametric form for the joint distribution $\P$ of the outcome and its information-driven forecasts. 
...
Theorem \ref{contraction} suggests that extremization can be beneficial even when the outcome is not binary. This may seem somewhat surprising because outcomes are generally not at the border of the range of possible forecast values.  Most importantly, however, the empirical success of extremization provides evidence in favor of the partial information framework because it can be explained and derived within the assumptions of the framework. 
