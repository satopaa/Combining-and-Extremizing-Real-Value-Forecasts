...
%
%These problems are avoided if 
 the individuals to make predictions in isolation and without being influenced by each other. These predictions are then combined 
%This way they do not need to be in the same location and can make their predictions on their own time without knowing anything about the others. 
% Regardless how this is done, t
 with a statistical aggregator that is nothing but a deterministic function from the predictions to  the consensus. This way the aggregator does not contribute any new information. Instead all information originates from the forecasters.  Given that the aggregator does not influence or have access to the internal mechanisms by which the individuals turn their information into overt predictions, it is the individual's responsibility to construct a prediction based on truthful and relevant information. Such ``honest'' predictions are considered to be \textit{information-driven}. The aggregator's responsibility, on the other hand, is to extract the maximum amount of information from the reported predictions and turn it into a consensus without any loss or distortion of information. If an aggregator achieves this, it is called \textit{efficient}. 
  Therefore collective forecasting and transforming the forecasters' information into a consensus becomes a joint effort between the aggregator and forecasters.


%This is the general viewpoint taken in the current paper. 
Even though the current paper explains how the forecasters can fulfill their responsibility, the main focus is on the aggregator. 
...
%In general, forecasts can be combined in many different ways.
 In practice by far the most common aggregator is the simple average. This popularity is due to its familiarity, simplicity, and good performance in 
% Panette could combine the predictions in many different ways. Most people would probably first consider the simple average. 
%has performed well
 in a variety of settings \citep{makridakis1983averages,clemen1986combining,clemen1989combining}.
% As long as the forecasts bracket the true outcome, that is, the outcome falls within their convex hull, averaging
...
%The main modeling differences are in the assumptions made about the structure of the forecasters' information.
 For instance, \cite{granger1989invited} and \cite{kim2001inefficiency} consider models in which each forecaster's information consists in equal parts of shared information (known to everyone) and private information (known only to one forecaster) about the (random) outcome. Surprisingly, both authors observe that the simple average of the forecasts is not efficient, that is, it is not the optimal aggregator under their models. This result became known as the \textit{inefficiency of the means}. 
...
Given, however, that it was discovered under specific models with unrealistic assumptions on the forecasters' information structure, it does not imply general inefficiency.
%that averaging is always inefficient. 
 
 
 In reality the forecasters can have different amounts of information, and not all information is either entirely public or private. For instance, the first two forecasters may share some information that the third forecaster does not know and so on. 
...
%This, however, is not the case.
  some previous authors have carried out such exhaustive analyses.  For instance, 
...
Our work maintains this level of generality but extends the inefficiency of the means in multiple dimensions. First, 
...
% and therefore apply to predictions of continuous (Gross Domestic Product, stock prices, product demand), binary (rain or no rain, candidate wins or loses), discrete (number of accidents, goals per season), and many other univariate types. 
 Second, the class of inefficient aggregators is grown from weighted averages to all measures of central tendency. 
...
Overall, our results are organized into four theorems that are summarized in the following enumeration.
...
Theorem \ref{contraction} shows that the weighted average violates several of the properties listed in Theorem \ref{optimal} and is inefficient as long as it assigns positive weight to at least two different forecasts. More specifically, it distorts the forecasters' information and hence leads to sub-optimal predictive performance.
...
\item Theorem \ref{contractionXphi} considers transformed weighted averages that first apply a strictly monotone transformation to the forecasts, averages them, and then transforms the average back to the original space. First, these aggregators distort the forecasters' information if the transformation is convex or concave and the averaging step assigns positive weight to at least two different forecasts.
...
Therefore well-known aggregators such as the harmonic, geometric, quadratic, and cubic weighted means are not efficient. Second, under mild regularity conditions, transformed weighted averages do not combine non-negative forecasts efficiently as long as positive weight is assigned to at least two different forecasts. This shows, among others, that it is not efficient to transform probability forecasts to 
% average 
%
...
The next section describes the theoretical framework for our analysis, constructs the outcome and its information driven forecasts, defines the class of efficient aggregators, and discusses some common properties of this class. Section \ref{MeasuresOfCentralTendency} explores the inefficiency of the measures of central tendency with the theorems described in the previous subsection.  After most common aggregators have been deemed in efficient, Section \ref{practiceEfficient} explains two general recipes for constructing efficient aggregators in practice. The final section concludes with a summary and discussion of our current and future work.
...
%describes
Information is described in terms of $\sigma$-fields (see \citealt{herves2013information} for a thorough discussion on why $\sigma$-fields are an appropriate description of information). More specifically, the principal $\sigma$-field $\F$ represents all possible information that can be known about $Y$. The $j$th forecaster knows only some subset  $\F_j \subseteq \F$ of this information. No assumption as to how the forecaster acquires this information is needed. For instance, it could  stem from books, news articles, personal recollections, and so on. These details are not relevant for aggregation and hence can be abstracted away. 
...
Based on $\F_j$ the forecaster forms a conditional distribution $\P(Y|\F_j)$ of the outcome. Given that in this paper the predictions are assumed to be univariate, the forecaster must summarize $\P(Y|\F_j)$ with a single value. 
...
%\marginpar{There are of course many different ways the forecaster could report the information Y | F. WE have decided to work with calibration because there is much literature on it and poeple know how to improve achieve or improve it in many different context.}
 ...
 If, however, one is left with uncalibrated forecasts, they can be calibrated in different ways depending on the circumstances. If the forecaster has made predictions in the past such that a stream of realized outcome-forecast pairs $(y_k, x_k)$ for $k \in \mathbb{N}$ is available, a new forecast $x$ can be empirically calibrated by replacing it with the average of $\{y_k : x_k = x, k \in \mathbb{N}\}$. If no past observations are available but one can construct a reasonable joint distribution $f(y,x)$ for the forecast-outcome pairs, then a new forecast $x$ can be calibrated by replacing it with $\E(Y|X = x)$ under the chosen joint distribution. In the most challenging case with no past observations or expertise to construct a joint distribution,
...
The next theorem describes this behavior by enumerating several common properties of the efficient aggregators.
...
In practice, the most common aggregators come from the family of measures of central tendency. These by definition aim to summarize the forecasts with a single central value. Therefore, at least intuitively, they can be expected to be closer to $\mu_0$ than the most variable forecast, and the resulting variance is likely to be too low for efficiency.
...
Overall, this class is large yet specific enough for detailed and useful results. These are given in the following theorem that analyzes $\mathcal{X}_w$ in the light of the efficiency properties listed in Theorem \ref{optimal}. Note that any deviation from those properties indicates inefficiency. 
...
\item $\mathcal{X}_w$ is not calibrated, that is, $\P\left[\E(Y | \mathcal{X}_w) \neq \mathcal{X}_w\right] > 0$ if there exists a forecast pair $i \neq j$ such that $\P(X_i \neq X_j) > 0$ and $w_i, w_j > 0$. In words, $\mathcal{X}_w$ is necessarily uncalibrated if it assigns positive weight to at least two different forecasts.  \label{XwCalib}
...
%
 ...
 If $\mathcal{X}_w$ is efficient, this must occur under a probability model where $\mathcal{X}'' = \beta_1X_1 + \beta_2X_2$ for some constants $\beta_1, \beta_2 \in \R$.  
...
Denote $\delta_j := \Var(X_j)$ and $\rho := \Cov(X_1, X_2)$. The coefficients can then be written as  %By minimizing the expected quadratic loss and using the fact that $\Cov(X_j, Y) = \delta$ (see \citealt{satopaamodeling} for more details), it is possible to show that
...
% are typically not available in practice due to company confidentiality or the forecasters' inability to identify the knowledge leading to their opinions.
  Therefore $\mathcal{X}'$ is not fully attainable.  Every aggregator, however, should aim to be as close to it as possible. 
...
An unconstrained linear aggregator can play with the tradeoff between shared and private  information. By doing this, it moves closer to the hypothetical optimum $\mathcal{X}'$ that can separate private and shared information.  Figure \ref{Intuition} illustrates the tradeoff under $N = 3$ forecasters, each indexed by a different color. 
...
Suppose $\Omega = \{\clubsuit, \heartsuit, \diamondsuit, \spadesuit\}$ and let $\F = 2^\Omega$ be all subsets of $\Omega$. The outcome is $Y = \one_A$, where $A = \{\heartsuit, \spadesuit\}$, and $\P$ is uniform over $\Omega$, that is, $\P(E) = |E|/4$ for all $E \in \F$. Consider two forecasters with the information sets $\F_1 = \sigma(\{\clubsuit\}, \{\heartsuit\})$ and $\F_1 = \sigma(\{\diamondsuit\}, \{\spadesuit\})$. In other words, the first forecaster knows whether the suit is Clubs or Hearts. The second forecaster, on the other hand, knows  whether the suit is Diamonds or Spades. Thus no forecaster is redundant, and their joint information is more than either of the individual information sets. In fact, together they know everything $\F'' =  \sigma(\{\clubsuit\}, \{\heartsuit\}, \{\diamondsuit\}, \{\spadesuit\}) =  \F$. The relevant predictions are
\begin{align*}
...
%To illustrate, observe that the forecaster with partial information $\F_j \subseteq \F$ has a conditional distribution $\P(Y | \F_j)$.
 In this paper the forecaster reports the expected value $\E(Y|\F_j)$ of $\P(Y | \F_j)$. Even though this is the natural starting point, it would be interesting to analyze other choices such as the median or $95\%$ credible interval. One could also examine the efficiency of popular techniques to capture other aspects of the outcome. For instance, the \textit{Newsvendor model}, that is widely used in operations management and applied economics, requires an estimate of  the variability of the outcome \citep{stevenson2002operations}. This is often estimated with the sample variance of the individual forecasts $\mathcal{S}_N = \sum_{j=1}^N (X_j - \bar{X})^2/(N-1)$. Is this efficient? In other words, does the sample average use all of the forecasters' information and hence equal to $\Var(Y|X_1, \dots, X_N)$ under some probability model?  Intuitively, given that $\Var(X_j) \leq \Var(Y)$ for all $j = 1, \dots, N$, the sample variance $\mathcal{S}_N$ is likely to underestimate $\Var(Y)$. 
 More specific direction of the bias could be explored by extending the common notion of (expectation) calibration to variance calibration: an estimate $\mathcal{S}$ of $\Var(Y)$ is calibrated if $\Var(Y|\mathcal{S}= s) = s$ for all $s \geq 0$.
...
% The analysis then proceeds to the analysis of the set $\mathcal{C}_3 = \{C : \mathcal{X}''(C) = \min(\mathcal{X}''(C)), C \in \tau \setminus (\mathcal{C}_1 \cup \mathcal{C}_2)\}$.
  This process can be repeated until one encounters a set $\mathcal{C}_k$ on which some forecasters disagree. Such a set must exist because at least two forecasters are assumed to disagree with positive probability.  This then gives the contradiction as argued before. 
