...
%The main modeling differences are in the assumptions made about the structure of the forecasters' information.
 For instance, \cite{granger1989invited} and \cite{kim2001inefficiency} consider models in which each forecaster's information consists in equal parts of shared (known to everyone) and private  (known only to one forecaster) information about the outcome. Surprisingly, both authors observe that the simple average of the forecasts is not efficient. In other words, it is not  optimal and hence can be improved upon. This result, known as the \textit{inefficiency of the means}, 
...
%The main limitation of this work stems from its restrictive assumptions on the forecasters' information structure: all information is known either by everyone or by only one forecaster. Due to its tractability, this \textit{symmetric} information model has become the popular choice in the literature (see, e.g., \citealt{ottaviani2006strategy, lichtendahl2013wisdom, palley2015eliciting}).
 relies on very specific models with unrealistic assumptions on the forecasters' information structure. Therefore it does not imply general inefficiency.
...
Theorem \ref{contraction} shows that any non-trivial weighted average violates several of the properties listed in Theorem \ref{optimal} and is therefore inefficient. By non-trivial we mean that the average assigns positive weight to at least two different forecasts. More specifically, such a weighted average distorts the forecasters' information and hence leads to sub-optimal predictive performance.
...
\cite{dawid1995coherent} call such pairs \textit{compatible} and give some results on how to construct one from the other. Our focus, however, is not on any specific pair. Instead our goal is to investigate whether there exists a probability space under which a given aggregator is efficient. If there is not, then this aggregator does not align with Definition \ref{infoAgg} and hence is always \textit{inefficient}. 

To show these kinds of inefficiency results directly with Definition \ref{infoAgg} can be challenging. An easier approach is to first understand the common behavior of all efficient aggregators and then check whether a given aggregator, such as the weighted average or median, agrees with it.  
...
In particular, item \ref{XwCalib}) shows that $\mathcal{X}_w$ is almost never calibrated. Therefore it is not based on any correct information about $Y$ even though the forecasters only give it correct information. This means that weighted averaging somehow distorts the forecasters' information. 
