...
In the past this result has been believed to arise because the average represents the forecasters' shared information too many times \citep{kim2001inefficiency,palley2015eliciting}. We, however, explain that this is not the reason. In fact,  
...
The next section describes the theoretical framework for our analysis, constructs the outcome and its information driven forecasts, and discusses some properties that all efficient aggregators have in common. Section \ref{MeasuresOfCentralTendency} discusses the inefficiency of the measures of central tendency and builds intuition on why the weighted average is inefficient. Section \ref{practiceEfficient} explains two general recipes for constructing efficient aggregators  in practice. The final section concludes with a summary and discussion of our current and future work.
...
To begin, consider some probability model $(\Omega, \F , \P)$ and the set of all random variables $X \in \F$ with a finite $p$th moment $\mathcal{L}^p  := \{ X \in \F : \E[|X|^p] < \infty\}$. Of particular importance is $\mathcal{L}^2$ because it contains all random variables with a finite mean and variance. Now, suppose the goal is to predict the value of $Y \in \mathcal{L}^2$
...
% probability space
%$(\Omega, \F , \P)$.
Even though the outcome $Y$ is defined on $(\Omega, \F , \P)$, the main results in this paper do not depend on any specific probability model but instead apply to all such models jointly. The only assumption is the existence of a probability model. This, however, cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].''  
%The set $\Omega \neq \emptyset$ represents all possible states of nature and $\F = \mathcal{P}(\Omega) = \{A : A \subset \Omega\}$ is the set of all subset of $\Omega$. 

%\marginpar{This could also be said by $X \in L^2$; Define this with general $p$ and explain that $p1$ is contained in $p2$.}

Suppose now that there are $N \in \mathbb{N}$ forecasters predicting $Y$. Denote the $j$th forecaster's prediction with the random variable $X_j \in \F$, $j = 1, \dots, N$. Akin to the previous studies discussed in Section \ref{Contributions},
...
Calibration turns out to be precisely the minimal condition that the forecasters must satisfy in order to fulfill their responsibility in collective forecasting. More specifically, 
\citet{satopaamodeling} show that a forecast $X$ is calibrated if and only if it is based on some partial information set, that is, $X = \E(Y|\G)$ for some $\G \subseteq \F$. This is convenient because instead of somehow trying to identify true information and turning that into a prediction, the forecasters can focus on the more concrete goal of being calibrated.
...
To illustrate, consider a calibrated probability forecast. If it equals $p$, then the outcome happens $100p\%$ of the time. This is precisely how probabilities are generally understood. Most forecasters with a basic understanding of probabilities are likely to target good calibration even without being instructed to do so. In fact, researchers have found many calibrated subpopulations of experts, such as meteorologists \citep{murphy1977reliability}, experienced tournament bridge players \citep{keren1987facing}, and bookmakers \citep{dowie1976efficiency}. On the more theoretical side, \cite{dawid1982well} show that an internally consistent or \textit{coherent} Bayesian can expect to become calibrated under feedback. \cite{foster1998asymptotic} show that the forecaster needs to know almost nothing about the outcome in order to be calibrated. 
...
Intuitively, this includes all information that can be inferred from the reported predictions. An aggregator is considered efficient if there exists some probability model under which it  construct the consensus based on all of $\F''$. This is  made precise in the following definition.
...
This inefficiency is sometimes believed to arise from the fact that $\mathcal{X}_w$ represents the shared information too many times \citep{kim2001inefficiency, palley2015eliciting}. The following example, however, builds intuition and shows that the explanation is somewhat more complicated than that. For the reader's convenience all technical derivations required for this example have been deferred to the Appendix.
...
Denote $\delta_j := \Var(X_j)$ and $\rho := \Cov(X_1, X_2)$. The coefficients can then be written as  %By minimizing the expected quadratic loss and using the fact that $\Cov(X_j, Y) = \delta$ (see \citealt{satopaamodeling} for more details), it is possible to show that
 $\beta_j = (\delta_1\delta_2 - \rho \delta_i)/(\delta_1\delta_2 - \rho^2)$, where $j,i \in \{1,2\}$ and $j \neq i$. This shows that if the experts' information sets $\F_1$ and $\F_2$ are independent, $\rho = 0$ and $\mathcal{X}'' = X_1 + X_2$. In other words, independent information combines via simple addition. To give intuition for this, consider two real-estate agents $A$ and $B$ who are valuing an apartment. Agent $A$ only sees one room and based on it values the apartment at $\$ x_A$. Agent $B$, on the other hand, sees a different room and  gives a valuation of $\$x_B$. If the agents know nothing else about the apartment except the rooms they visited, their information sets are disjoint and their combined information gives a valuation of $\$(x_A + x_B)$. This is also not in contradiction with the general practice of averaging. To see this, suppose one forecaster knows everything that the other forecaster knows. In such a setting it is unlikely that any rational advocate of averaging would assign anything but weight one to the more knowledgeable forecaster. Therefore unique information should receive weight one.
%  combined 
...
%To illustrate, observe that the forecaster with partial information $\F_j \subseteq \F$ has a conditional distribution $\P(Y | \F_j)$.
 In this paper the forecaster reports the expected value $\E(Y|\F_j)$ of this distribution. Even though this is the natural starting point, it would be interesting to analyze other choices such as the median or $95\%$ credible interval of the conditional distribution $\P(Y | \F_j)$. One could also examine the efficiency of popular techniques to capture other aspects of the outcome. For instance, the \textit{Newsvendor model} that is widely used in operations management and applied economics often needs an estimate for the variance of the outcome \citep{stevenson2002operations}. Typically the sample variance of the individual forecasts is used. Does this approach use all of the forecasters' information and hence equal to $\Var(Y|X_1, \dots, X_N)$ under some probability model? If not, what kind of bias is there? 
...
% 
  In terms of other applications, our results could shed light on estimation of tail risk in asset management or information collection in prediction markets.
