...
Even though this framework is extremely general, the authors focus mostly on the specific task of aggregating probability forecasts. In particular, their work does not involve any theoretical analysis of weighted averaging. 

\marginpar{Must say that PIF is used}

Such an analysis is one of the main contributions of the current paper. More specifically,
...
the analysis extends the result
...
provides an original point of view on forecast aggregation and introduces general, yet intuitive, descriptions of well-known properties
...
In the probability forecasting literature the under-confidence of a simple aggregator, such as the average forecast, is typically alleviated by \textit{extremizing}, that is, by systematically transforming the aggregate towards its nearer extreme (at zero or one). 
...
Extremization is particularly easy to understand in the binary outcome case where the most confident values, namely zero and one, are explicit boundary points of the sample space. Naturally, the same applies to probability forecasts of any categorical outcome. 
...
%    A similar story applied to any categorial outcome with forecasts in the corresponding simplex. 
    However, if both the outcome and the forecasts are real-valued, it is not clear anymore what values represent the most confident forecasts. Therefore extremization does not have an explicit target, and, consequently, it is not at first  clear what it means or how it could be applied. This may be the main reason why, to the best of our knowledge, no previous literature has discussed extremizing real-valued forecasts. 
    %     such a technique a) would work, and b) can even be applied to real-valued forecasts.
      Our second contribution, however, shows that extremizing can improve an aggregator also when the individual forecasts are real-valued. First, the paper motivates and generalizes the aforementioned extremization of probability forecasts to any univariate forecasts. Second, extremization of real-valued forecasts is illustrated with a convex optimization procedure that linearly extremizes the optimally weighted average. This technique is applied to simple examples involving both synthetic and real-world data. In both examples extremizing leads to an aggregate that is not only more resolute and reliable but also exhibits many properties of optimal aggregation.
