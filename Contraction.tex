...
\author{Ville A. Satop\"a\"a\thanks{Ville A. Satop\"a\"a is a Statistician, Department of Technology and Operations Management, INSEAD, Boulevard de Constance, 77305 Fontainebleau CEDEX, France (e-mail: ville.satopaa@insead.edu)}
}
%\date{\vspace{-6.5ex}}
\date{\today}
%%%%%% Begin document with header and title %%%%%%%%%
...
There are, of course, many different ways one can combine predictions into a single representative value. In practice, most people would probably consider averaging first. 
% Panette could combine the predictions in many different ways. Most people would probably first consider the simple average. 
 Fortunately, simple averaging has been shown to perform remarkably well in a variety of settings (see, e.g., \citealt{clemen1986combining, clemen1989combining, makridakis1983averages})
...
%The main limitation of this work stems from its restrictive assumptions on the forecasters' information structure: all information is known either by everyone or by only one forecaster. Due to its tractability, this \textit{symmetric} information model has become the popular choice in the literature (see, e.g., \citealt{ottaviani2006strategy, lichtendahl2013wisdom, palley2015eliciting}).
 Of course, in reality the forecasters can have different amounts of information, and not all information is either entirely public or private. For instance, the first two forecasts may share some information that the third forecaster does not know and so on. Therefore different parts of information can be shared by different subgroups of the forecasters, and the forecasters information can be seen as a complex network of partially overlapping sets. 
...
Analyzing aggregation across all possible information structures may at first appear intractable. Previous work, however, has shown this to not be the case.
...
For instance, \cite{dawid1995coherent} show that the weighted average of two probability forecasts of a future event is inefficient.
%This result, however, is not directly useful for many real-world forecasting applications because these often involve a large number of forecasters.
 \cite{Ranjan08} then generalize this result to $N \in \mathbb{N}$ probability forecasts of a future event. 
%These authors also make no assumptions about the underlying probability model. 
%Therefore the results are very general. 
Even though these authors do not focus on the role of information, their forecasters can be shown to be operating on different information sets. The structure of these information sets, however, is in no way restricted.  Instead the results are derived jointly under all possible information structures. In addition to this, the authors assume nothing about the underlying probability model except its existence. Therefore their results hold  jointly across all probability models and all information structures.
...
This paper generalizes the inefficiency result even further. 
...
% but also applies to many other popular aggregators. 
 To illustrate, suppose that all the analysts independently predict a probability of $0.9$ that Bin Laden is in the compound. If these forecasts are based on somewhat different information, then their combined information should lead to an aggregate greater than $0.9$. In this simple scenario, however, all measures of central tendency (mean, median, mode, geometric mean, harmonic mean, trimmed mean, and so on) aggregate to $0.9$. Therefore they cannot be using all the forecasters' information. Even though this is admittedly a very contrived example, it does suggest an interesting question: are measures of central tendency inherently inefficient in aggregating forecasts?
...
Our main research objective is to answer this question. Unlike these previous studies that have largely focused on probability forecasts of binary outcomes, our analysis leaves the type of the outcome unspecified. Therefore the results apply to predictions of continuous (Gross Domestic Product, stock prices, product demand), binary (rain or no rain, candidate wins or loses), discrete (number of accidents, goals per season), and many other univariate types. The analysis is carried out under the general framework, known as the \textit{partial information framework}, introduced by \cite{satopaamodeling2, satopaamodeling} as a platform for analyzing information-driven forecasts. 
%This framework
%%
% explicitly notates and describes the forecasters' information without constraining its structure. Furthermore, it assumes nothing about the underlying probability model except its existence. Therefore it is particularly well-suited for deriving highly general results about forecasting.
  Similarly to  \cite{dawid1995coherent} and \cite{Ranjan08}, this framework makes no assumptions about the forecasters information structure or about the underlying probability model. 
...
%   Unlike these previous studies, however, the type of the outcome is not specified. 
 Therefore our results are more general that previous studies in more than one dimension. The following list gives a more detailed explanation of our contributions.
...
Denote the outcome of interest with $Y$. Suppose this outcome has a finite mean $\E(Y) := \mu_0 < \infty$ and a finite variance $\Var(Y) := \delta_0 < \infty$. The partial information
framework assumes that $Y$ is random variable under some
 probability space
$(\Omega, \F , \P)$. Intuitively, this space is a probabilistic model for the way $Y$ behaves in practice. The main results in this paper do not rely on a specific instance of $(\Omega, \F , \P)$ but instead apply to all probability models jointly. The only assumption is the existence of a probability space. This, however, cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].''  
%The set $\Omega \neq \emptyset$ represents all possible states of nature and $\F = \mathcal{P}(\Omega) = \{A : A \subset \Omega\}$ is the set of all subset of $\Omega$. 
...
Our analysis adopts this viewpoint but,
...
%In the past many fields, including statistics, finance, and economics, have used $\sigma$-fields as a reasonable description of information
In this way the principal $\sigma$-field $\F$ holds all possible information that can be known about $Y$, and the $j$th forecaster predicts $X_j = \E(Y | \F_j)$ based on some partial information set $\F_j \subseteq \F$. 
...
Given that $\E(Y|Z_j) = \E(Y | \F_j)$ for $\F_j = \sigma(Z_j)$, this formulation is essentially equivalent to the one used by the previous studies. The only difference is that our formulation excludes the extra random variable $Z_j$ and hence makes it more clear that nothing is assumed about the sources of the information. For instance, the forecaster's information could stem from books, news articles, personal recollections, and so on. All these details are not relevant for aggregation and hence have been abstracted away.
...
In practice the forecasters may not use all the information they know about $Y$. For instance, they may not be able to  reconcile all the available information or simply may decide to conceal some. In the end of the day, the aggregator only gets to operate on the reported forecasts and the
...
Consider $N$ forecasts $X_j$ of the outcome $Y$. An aggregator $\mathcal{X}''$ is called efficient if there exists a probability space $(\Omega, \F, \P)$ under which $\mathcal{X}'' = \E(Y | X_1, \dots, X_N)$ or, equivalently, $\mathcal{X}'' = \E(Y | \F'')$ with $\F'' = \sigma(X_1, \dots, X_N)$. 
...
\cite{dawid1995coherent} call such pairs \textit{compatible} and give some results on how to construct one from the other. At this point, however, our focus is not on any specific pair. Instead, our analysis investigates whether there exists any probability space under which measures of central tendency are efficient. If there is not, then these aggregators do not satisfy the condition in Definition \ref{infoAgg} and hence are deemed \textit{inefficient}.
...
In order to better understand efficient aggregators, the next theorem enumerates some of their properties. 
...
This is the intuition motivates the more rigorous analysis in the following sections. 
...
This section continues and generalizes the analysis by \cite{dawid1995coherent} and \cite{Ranjan08} of the most commonly used aggregator, namely the weighted average. More precisely, the focus is on the following class of aggregators.
...
The most famous member of this class is the equally weighted average $\frac{1}{N}\sum_{j=1}^N X_j$ that sets all $w_j = 1/N$. Overall, the class of weighted averages is large yet specific enough for us to prove some detailed and useful results. These are given in the following theorem that analyzes $\mathcal{X}_w$ in the light of the efficiency properties listed in Theorem \ref{optimal}. 
...
This section expands Definition \ref{Xw} to allow transformations of the forecasts before they are averaged. Typically the goal is to transform the forecasts $X_j$ to some other space where they are more symmetric, possibly around their mean \citep{baron2014two}. The following definition makes this class of aggregators precise.
...
\end{class} 
The function $\Phi$ is strictly monotone and hence invertible so that its inverse exists and two different forecasts are not mapped to the same value. From an information perspective, an invertible function preserves information; that is, $\sigma(X_j) = \sigma(\Phi(X_j))$. Clearly, if $\mathcal{X}_\Phi$ is to be efficient, the transformation $\Phi$ cannot lose any of the original information content. Thus, invertibility is essential.  Despite this requirement, however, the following theorem shows that many transformed weighted averages are inefficient. 
...
If $\mathcal{X}_\Phi$ assigns positive weight to at least two different forecasts, then it has the following properties. 
\begin{enumerate}[i)]
\item If $\Phi$ is convex or concave, then $\mathcal{X}_\Phi$ is not calibrated and hence not efficient. Furthermore, if $\Phi$ is strictly convex or concave, then  $\mathcal{X}_\Phi$ is not even marginally consistent.\label{convex} 
\item If  $X_j \geq 0$ and $\Phi: \R^+ \to S \subseteq \R$ is  continuous, then $\mathcal{X}_\Phi$ is inefficient. \label{mono}
\end{enumerate}
\end{theorem}
...
This paper discussed forecast aggregation under a general probability model, called the partial information framework. The forecasts were assumed to be information-driven but no restrictions were placed on the structure of the forecasters' information. The discussion began by defining efficient aggregators (Definition \ref{infoAgg}) and then enumerating  several of their properties (Theorem \ref{optimal}). These properties provide guidance for developing and understanding aggregators that are often used in practice. In this paper they shed light on different  measures of central tendency. The analysis progressed in steps of increasing generality. First, the focus was on the class of weighted averages.  
...
&= \E\left[ 2 \mathcal{X}_\Phi (\Phi(X_j)- \min \{\Phi(X_j)\} + \epsilon)  \frac{\partial }{\partial x}\Phi^{-1} \left( x \right) \bigg|_{\sum_{j=1}^N w_j \Phi(X_j )}   \right] \label{partDer}
%&> 0
\end{align}
Given that $\Phi(X_j)- \min \{\Phi(X_j)\} + \epsilon > 0$, $\frac{\partial }{\partial x}\Phi^{-1} \left( x \right) > 0$, and $\mathcal{X}_\Phi \geq 0$, this partial derivative is positive except in the degenerate case where $\mathcal{X}_\Phi = 0$ almost surely. This, however, implies that $\E(\mathcal{X}_\Phi) = \mu_0 = 0$. Given that each $X_j \geq 0$ is marginally consistent, $\E(X_j) = 0$. This is only possible if $X_j = 0$ almost surely and for all $j = 1, \dots, N$. This contradicts the theorem hypothesis that states that $\mathcal{X}_\Phi$ assigns weights to at least two different forecasts. Thus it must be the case that $\P(\mathcal{X}_\Phi > 0) > 0$, forcing the partial derivative (\ref{partDer}) to be positive.  
