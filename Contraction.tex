...
%Given that the weighted average is under-confident, a natural improvement is to post-transform it closer to its nearer extreme (at zero or one).  This technique, known as \textit{extremizing}, is widely used to 
%
%To address this sub-optimality, they propose a beta transformation that moves the weighted average closer to its nearer extreme (at zero or one). This is an example of a more general approach,  known as \textit{extremizing}, that 
%
%
%The amount of transformation is typically determined by minimizing a proper scoring rule, which addresses calibration and sharpness simultaneously.  
%
% This technique, known as \textit{extremizing}, is widely used to  

%Therefore these weighted averages can be regarded as suboptimal. 
...
% Therefore  the result in \cite{Ranjan08} is a special case of this more general theorem. 
...
%Finally and most importantly, the analysis provides concrete guidance for developing new aggregators in the future. 
...
%\marginpar{The idea is to bring together / formalize / generalize / unify many of the ideas, and explain intuition behind some mysterious techniques. }
...
%\marginpar{Should I state here what the intuition is?}

%Even though many different types of outcome-forecast pairs are possible, the two most common ones are probability forecasts of binary events, such as rain or no rain tomorrow, and point forecasts of continuous quantities, such as tomorrow's stock price. 

%In the recent years, due to modern social and computer networks  that permit the collection of a large number of responses from both human and machine agents. This has given rise to crowdsourcing platforms, such as MTurk and Witkey, and many companies, such as Myriada, Lumenogic, and Inkling, that have managed to successfully  capitalize on the benefits of collective wisdom. 



%
%- Aggregation of different forecast is important for decision making and accuracy\\
%- Different forms of averages are typical.\\
%- Many noticed that these are suboptimal, i.e. under confident. Explanations in clyde Barons paper
%- Most of the studies have focused on prob forecasting where many of these concepts are intuitvel easier to grasp.\\
%- The main contribution of this paper is a theorem that proves the subopitmality of any weighted average on any type of forecast. Generalizes Gneiting. Explain other contributions.\\
%
%\marginpar{This is all somewhat obvious in probability forecasting. However, this all is not so obvious in continuous. We show general results. It may be obvious because we are dealing with probabilities. Then, 1/2 can be said to be non-informative: flat distribution. Becomes a bit more complicated when continuous. }

%\section{Previous Work} \label{probability}
%Explain here that the motivation for us comes from prob forecasting and from ranjnans paper. Explain all the terms, sharpness, calibration, etc. There are equivalents to all of these.

\section{Forecast and Aggregation Properties} \label{propertiesS}
\subsection{Optimal Aggregation}

%\marginpar{perhaps start here by saying that Y is outcome, X is an individual forecast, and Z is an aggregate forecast. Notation!!}
...
%If $X_j$ is reliable, then $X_j = \E(Y | \F_j)$, where $\F_j := \sigma(X_j) \subseteq \F$ is the $\sigma$-field generated by $X_j$. 
%From the aggregation perspective, no generality is lost by assuming that $\F_j$ is the information used by forecaster $j$. 
...
%Overall, this setup is a direct generalization of the setting considered in \cite{Ranjan08}.
...
%See, e.g., Durrett Theorem 5.1.8. 
%The following contraction theorem compares the revealed aggregator and the weighted average. 
%shows that a weighted average is always under-confident in a sense that it is closer to the marginal mean $\mu_0$ than the revealed aggregator $X''$. 
...
%Among the properties listed in Theorem \ref{properties} only the third property is restricted to aggregators. In other words, it is irrelevant to the individual forecasts.  
...
%These two cases form the logical extremes of a spectrum of information that is quantified by variance. 
...
%
%is common among aggregators that collect information. In this paper such aggregators are called 

%Aggregators are generally thought of as pooling the information from the different sources of forecasts.
...
%\marginpar{I need to make clear the connection of resolution here: resolution is probably better seen as how closely the forecast is to Y and its variance than how close it is to mu0. Converges in distribution to Y as information increases}

%
%resolution states that $\mathcal{X}''$ is always at least as confident as the most confident forecaster. Furthermore, the variance of $X''$ is upper bounded by the variance of $Y$. This upper bound holds for all reliable forecasts. The proof follows directly from the fact that conditional expectation is a contraction in $L^2$ \citep[Theorem 5.1.4.]{durrett2010probability}. Therefore, similarly to probability forecasting, a reasonable goal in the general aggregation setting is to maximize resolution subject to reliability. To put in information terms, to find an aggregator that is consistent with as much information as possible. 
...
\end{enumerate}
\end{theorem}
%\marginpar{can we prove Gnetigin equivalent of the under confidence result. Iti s a bit different to ours.}
...
% it assigns all weight to the most informed forecaster who also happens to use all the information that the other $N-1$ forecasters use. 
...
%However, even if $\mathcal{X}_w$ was reliable, its variance is too low. 
...
%\marginpar{linear opinion pool is a very poor term as it really should be convex opinion pool}
...
%\marginpar{can we expand on the median idea. not too important}

%More specifically, if $\F_i \subseteq \F_j$, then $\Var(X_i) \leq \Var(X_j)$ \citep{satopaamodeling2}.



%\marginpar{Baron discuss psychological reasons for extremization. Our paper will provide the mathematical explanation.}

%\noindent
%In addition to generalizing the Gneiting's result, our approach gives intuition on why measures of central tendency are not appropriate. This theorem raises several important points:
%\begin{enumerate}
%%\item[a)] Recall that variance under the partial information model implies higher information. In other words, variance parametrizes the amount of information used. The amount of information in a linear opinion pool is bounded from above by the amount of information of the most informed forecaster. This is not how information aggregation should operate. Information aggregation should collect information from all the forecasters and hence have at least as much information as the most informed forecaster. 
%\item[b)] 
%%\item[c)] This provides intuition why measures of central tendency are poor in aggregation: they are at most as variable as the most variable forecast, i.e. they reduce variance. This is seen as poor information aggregation.
%%\item[d)] In probability forecasting the goal is to maximize sharpness subject to being calibrated. This is generalized by the general partial information framework which states that the goal is to maximize variance subject to being conditionally unbiased.   All calibrated forecasts have variance less than Y
%%\item[e)] Simple average can be seen as a reasonable estimate of $\mu_0$. This targets the non-informative prior, which suggests that the mean should be under-confident. In fact, can view as a weighted average (precision weighted). 
%\end{enumerate}

%\marginpar{Explain that median is upped bounded.}
%\marginpar{We need a reference that says that all measures of central tendency reduce variance}


\section{Extremizing Continuous Forecasts} \label{extremization}
%Given that the weighted average  is under-confident, a reasonable approach is to shift it (or some other measure of central tendency) away from the prior mean $\mu_0$. This technique has been used largely in the probability forecasting literature. For instance, \cite{Ranjan08} suggest using 
%\begin{align*}
%p_k &= H_{\alpha, \beta} \left( \sum_{i=1}^k w_i p_{ik}\right) &\text{ for } k = 1, \dots, K,
%\end{align*}
%$H_{\alpha, \beta} \left( \cdot \right)$ is the CDF of a beta density. They constrain $\alpha = \beta \geq 1$, which performs nothing but extremization from $\mu_0 = 1/2$. This choice of prior is the generic non-informative choice. However, in many applications, the event occurrence may not be equivalent to a coin flip. For instance, the chance of rain typically has a prior mean much lower than $1/2$. Therefore the extremization technique should at least facilitate the option of estimating this point from the training set. 



%\marginpar{Add more citation on extremization}
%\marginpar{Write about the weighted average and how to go about getting it}
...
%The variables $Y_k$ and $X_{jk}$ are considered independent and identically distribution across the sub-index $k$. In particular, 
...
\end{align*}
% \marginpar{This has a very similar form to the gaussian rev aggre}
...
%The estimation problem is given by
\begin{align}
 \label{firstProblem}
 \begin{split}
...
\end{align}
%\begin{align*}
%\argmin_{\alpha, \mu_0} \sum_{k=1}^K \left\{\alpha  \left( \sum_{i=1}^k w_i X_{ik} - \mu_0\right) + \mu_0 - Y_k \right\}^2,
%\end{align*}
%The solution does not necessarily lead to extremization. In fact, if $\alpha \in [0,1)$, then $\w' \X_k$  is contracted towards the marginal mean $\mu_0$. This option is allowed for the sake of illustrating later on in the paper that $\alpha > 1$ is a consequence of the data instead of an explicit optimization constraint. 
...
%To obtain the regular OLP, add constraints $\beta_0 = 0$ and 
%\marginpar{In the absence of any other information, the sample mean that one observes is the "best estimate" of the value of the population mean.}


%\marginpar{we will illustrate on continuous forecasts. Ranjan showed on probability forecasts. Can extend the idea of extremizing easily to any context. The idea is so simple!}
...
%\marginpar{Give an example box plot and argue that this model is very appropriate. This reminded me of the way Bayesian do visual evaluation.} 
...
%This aggregator simplifies under no information overlap and high information overlap to $\mathcal{X}_k'' = \sum_{j=1}^5 X_{jk}$ and $ \mathcal{X}_k'' = \left( \sum_{j=2}^5 X_{jk} \right) - 3X_{1k}$, respectively.
%%% as follows: 
%%\begin{align*}
%%\text{\textit{No Overlap: }} \mathcal{X}_k'' = \sum_{j=1}^5 X_{jk} && \text{  } &&
%%\text{\textit{High Overlap: }} \mathcal{X}_k'' = \left( \sum_{j=2}^5 X_{jk} \right) - 3X_{1k}
%%\end{align*}
%In both scenarios the variance of $\mathcal{X}'' $ matches the total amount of information known to the group of forecasters. Therefore $\mathcal{X}''$ uses all of the available information. 
%% In the NOV scenario $X_k'' = \sum_{j=1}^5 X_j$ whereas in the HOV scenario $X_k'' = \left( \sum_{j=2}^5 X_j \right) - 3X_1$. 
...
\end{enumerate}
%\textit{No Overlap} (NOV) and \textit{High Overlap} (HOV). Both scenarios fix $\delta_j = 0.1 + 0.02j$ for $j = 1, \dots, 5$. The NOV scenario sets $\rho_{i,j} = 0$ for all $i,j$ and hence assumes the forecasters to have independent information sources. As a group the forecasters know $80\%$ of the total information. 
%The HOV scenario introduces information overlap by setting $\rho_{i,j} = 0.12$ for all $i \neq j$. In this case the group's information is $32\%$ of the total information. Therefore the amount of information revealed by the forecasts is much less than in the NOV scenario. These two scenarios are illustrated in Figure \ref{Diagrams}. 
...
%Here E-OWA is given by $\mathcal{X}_k^*$ and OWA is equivalent to $\mathcal{X}_k^*$ under $\alpha = 1$. Therefore the fitting process for both of these aggregates is described in Section \ref{extremization}. 
% the aggregators are first trained on a set of $10,000$ draws from (\ref{NExperts}) and 
...
%Only predictions made on the testing set are included.
%However, to avoid choosing any specific set of bins, the outcome-forecast relationship is described with a smoothing spline. 
%More specifically, the forecasts-outcome pairs are first sorted into $R-1$ bins based on $R$ empirical quantiles of the forecast. Within each bin both the forecasts and the outcomes are averaged. The average forecasts across all bins are then plotted against their corresponding average outcomes.
...
%\marginpar{we can see here that this amounts essentially to X''}
%\marginpar{discuss these tables}

%% latex table generated in R 3.2.0 by xtable 1.7-4 package
%% Wed May 20 21:35:11 2015
%\begin{table}[ht]
%\centering
%\caption{\textit{High Overlap}. The estimated parameter values of optimally weighted average, $\mathcal{X}_w$ and the extremized version of the optimally weighted average, $\mathcal{X}^*$. }
%\begin{tabular}{lrrrrrrr}
%   \hline \hline
%Forecast & $\mu_0$ & $\alpha$ & $w_1$ & $w_2$& $w_3$& $w_4$& $w_5$ \\ 
%  \hline
%$\mathcal{X}_w$ &  &  & 0.0000 & 0.0000 & 0.0440 & 0.4262 & 0.5298 \\ 
%   $\mathcal{X}^*$ & -0.0077 & 1.3048 & 0.0000 & 0.0000 & 0.1456 & 0.3959 & 0.4585 \\ 
%   \hline
%\end{tabular}
%\label{HighParams}
...
%    Any deviation from the diagonal represents lack of reliability. The red curve within the inlined plot represents the marginal distribution of $Y$. The closer the histogram is to this distribution, the most resolute the corresponding aggregate is.
%   As a group the forecasters know $80\%$ of the total information. The variances of EWA, OWA, and E-OWA are $0.03$, $0.06$, and $0.77$, respectively. However, given that only E-OWA is reliable, only its variance can be interpreted as the amount of used information.
        \label{NOVerlap}
\end{figure}
...
%    Any deviation from the diagonal represents lack of reliability. The red curve within the inlined plot represents the marginal distribution of $Y$. The closer the histogram is to this distribution, the most resolute the corresponding aggregate is.
%    As a group the forecasters know at most $40\%$ of the total information. Therefore the variance of a reliable forecast is upper bounded by $0.4$. The variances of EWA, OWA, and E-OWA are $0.11$, $0.13$, and $0.26$, respectively. However, given that only E-OWA is reliable, only its variance can be interpreted as the amount of used information.
    }        
   \label{HighOverlap}
\end{figure}
...
%\begin{figure}[t!]
%   \centering
%   \includegraphics[width = \textwidth]{ReliabilityA} % requires the graphicx package
%   \caption{Scenario A: The forecaster have independent information.}
%   \label{reliabilityA}
%\end{figure}

%\begin{figure}[t!]
%   \centering
%   \includegraphics[width = \textwidth]{ReliabilityB} % requires the graphicx package
%   \caption{Scenario B: The forecaster have high information overlap.}
%   \label{reliabilityB}
%\end{figure}

%\marginpar{Describe the estimated values of w and alpha; why did Gneiting show this!}
%\marginpar{Of course, we could estimate the optimal here. This inoves estimating the entire covariance matrix which is challenging especially for large N. Here we reduce it to a linear number of parameters. Of course, requires a training set.}

%\marginpar{I think this is alrgith even though here revealed is a linear function. We are estimating the aggregator in very different way; that is, we are finding least squares estimate instead of a covariance matrix. This is quite different. and it is cool to see that we can find it this way.}
...
%In both scenarios the averaging aggregators $\bar{\mathcal{X}}$ and $\mathcal{X}_w$ are under-confident. 
...
%present the weights used in $\mathcal{X}_w$. They increase in $\delta_j$ and differ noticeably from the equal weights employed by $\bar{\mathcal{X}}$. These tables also present the parameter estimates for $\mathcal{X}^*$ whose weights are somewhat different from the ones used in  $\mathcal{X}_w$. The fact that 
 $\alpha > 1$ in both scenarios, reflecting the need to correct the under-confidence of the traditional weighted average. 
%Under no information overlap, E-OWA assigns almost equal weights to the forecasters and heavily extremizes the resulting average. Under high information overlap, on the other hand, E-OWA assigns unequal, increasing weights but extremizes less. 
...
%% latex table generated in R 3.1.2 by xtable 1.7-4 package
%% Thu May  7 14:20:07 2015
%\begin{table}[h!]
%\centering
%\caption{\textit{High Overlap.}  The average quadratic loss, $L(\Y,\boldsymbol{\mathcal{X}} )$ with its three additive components: reliability (REL), resolution (RES), and uncertainty (UNC). The rows have been ordered in terms of $L(\Y,\boldsymbol{\mathcal{X}} )$. The final column, $s(\boldsymbol{\mathcal{X}} )$ gives the sample variance of the forecast. } 
%\begin{tabular}{lrrrrr}
%  \hline \hline
%  Forecast & $L(\Y,\boldsymbol{\mathcal{X}} )$ & REL & RES & UNC & $s(\boldsymbol{\mathcal{X}} )$\\ 
%  \hline
...
%Among all the aggregators, $X_k''$ minimizes the expected quadratic loss that can be approximated with the average quadratic loss over the testing set.
...
%It is more reliable but less resolute than $\bar{\mathcal{X}}$. In general, $\bar{\mathcal{X}}$ is less reliable than all the other aggregators. Compared to $\bar{\mathcal{X}}$, the weighted average $\mathcal{X}_w$ presents some improvement, particularly in terms of reliability. All of these three aggregators, however, have a sample variance well below that of the best individual and are not expanding. 
...
%
%In the HOV scenario, on the other hand, E-OWA presents much lower resolution. This is expected because the group of forecasters in this scenario possess at most $40\%$ of the information. The variance of E-OWA is around $0.26$, suggesting that about $26\%$ of the total information is used in the aggregate. In both scenarios the variance of E-OWA is larger than the maximum variance among the individual forecasts, namely $\delta_5 = 0.2$. Therefore E-OWA is both reliable and expanding. 
%
%
%
%More specifically, in the NOV and HOV scenarios the variances of EWA are $0.03$ and $0.11$, respectively, and the variances of OWA are $0.06$ and $0.13$, respectively. These are all much lower than the maximum variance among the individual forecasts, namely $\delta_5 = 0.2$. Therefore EWA and OWA are neither reliable nor expanding. 
%
%The variance of E-OWA in this scenario is around $0.77$. Therefore almost all of the group's information is used in the aggregate. 
...
%
%serves as a guideline for understanding how well the different forecasters could predict $Y$ based on the predictors.
...
%         Evaluating aggregation of the forecasts from the models $\mathcal{M}_1$ and $\mathcal{M}_2$ that have non-overlapping predictor sets. 
%        Any deviation from the diagonal represents lack of reliability. The red curve within the inlined plot represents the marginal distribution of $Y$. The closer the histogram is to this distribution, the most resolute the corresponding aggregate is.
...
\begin{figure}
        ...
        ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc.
          %(or a blank line to force the subfigure onto a new line)
%        \begin{subfigure}[b]{0.323\textwidth}
%                \includegraphics[width=\textwidth]{IndependentE-ELP}
%                \caption{E-ELP}
%                \label{fig:tiger}
%        \end{subfigure}
        ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc.
...
\begin{figure}
        \centering
%        \begin{subfigure}[b]{0.323\textwidth}
%                \includegraphics[width=\textwidth]{DependentModel1}
%                \caption{$\mathcal{M}_1$}
%                \label{fig:gull}
%        \end{subfigure}%
%        ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc.
...
%        \end{subfigure}
        ...
        %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc.
          %(or a blank line to force the subfigure onto a new line)
%        \begin{subfigure}[b]{0.323\textwidth}
%                \includegraphics[width=\textwidth]{DependentE-ELP}
%                \caption{E-ELP}
%                \label{fig:tiger}
%        \end{subfigure}
         %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc.
        ...
        \end{subfigure}
%              \begin{subfigure}[b]{0.323\textwidth}
%                \includegraphics[width=\textwidth]{DependentModelF}
%                \caption{$\mathcal{M}_F$}
%                \label{fig:mouse}
%        \end{subfigure}  
...
%          
%          Evaluating aggregation of the forecasts from the models $\mathcal{M}_1$ and $\mathcal{M}_3$ that have overlapping predictor sets. 
%          Any deviation from the diagonal represents lack of reliability. The red curve within the inlined plot represents the marginal distribution of $Y$. The closer the histogram is to this distribution, the most resolute the corresponding aggregate is. 
          }
...
% present the weights used in $\mathcal{X}_w$. Given that these weights are  and explain that  $\mathcal{X}_w$ weights the two models almost equally, hence resembling $\bar{\mathcal{X}}$.  
%These tables also show that the weights are very similar under $\mathcal{X}^*$ and $\mathcal{X}_w$. 
...
%\marginpar{these estimates are what we would expect them to be. the mean of Y is around 35.81796. The weights are the same which shows that the procedure essentially computes the mean and then extremizes it. }

% latex table generated in R 3.2.0 by xtable 1.7-4 package
% Wed May 20 21:52:26 2015
%\begin{table}[ht]
%\centering
%\caption{\textit{High Overlap}. The estimated parameter values of optimally weighted average, $\mathcal{X}_w$ and the extremized version of the optimally weighted average, $\mathcal{X}^*$. }
%\begin{tabular}{lrrrr}
%   \hline \hline
%Forecast & $\mu_0$ & $\alpha$ & $w_1$ & $w_2$\\
%  \hline
%$\mathcal{X}_w$ &  &  & 0.5931 & 0.4069 \\ 
%  $\mathcal{X}^*$ & -37.6776 & 1.4382 & 0.5375 & 0.4625 \\ 
%   \hline
%\end{tabular}
%\label{HighParamsReal}
%\end{table}
...
\centering
\caption{The average quadratic loss, $L(\Y,\boldsymbol{\mathcal{X}} )$ with its three additive components: reliability (REL), resolution (RES), and uncertainty (UNC).The final column, $s(\boldsymbol{\mathcal{X}} )$ gives the sample variance of the forecast. 
%Only E-OWA is expanding. The sample variance of $Y$ is $279.08$, which upper bounds $s(\boldsymbol{\mathcal{X}} )$
} 
\begin{tabular}{llrrrrr}
  \hline \hline
Scenario &  Forecast & $L(\Y,\boldsymbol{\mathcal{X}} )$ & REL & RES & UNC & $s(\boldsymbol{\mathcal{X}} )$\\ 
  \hline
%$\mathcal{M}_1$ & 187.07 & 7.58 & 99.32 & 278.81 & 90.82 \\ 
%  $\mathcal{M}_2$ & 185.23 & 12.46 & 106.04 & 278.81 & 93.60 \\ 
%  EWA & 154.25 & 30.11 & 154.67 & 278.81 & 58.85 \\ 
%  OWA & 154.48 & 29.38 & 153.72 & 278.81 & 59.04 \\ 
%  E-OLP & 131.29 & 8.86 & 156.39 & 278.81 & 154.54 \\ 
%  $\mathcal{M}_F$ & 112.71 & 8.78 & 174.88 & 278.81 & 167.44 \\ 
...
%  Forecast & $L(\Y,\boldsymbol{\mathcal{X}} )$ & REL & RES & UNC & $s(\boldsymbol{\mathcal{X}} )$\\ 
%  \hline
%%  $\mathcal{M}_1$ & 189.77 & 6.46 & 95.50 & 278.81 & 92.26 \\ 
%%  $\mathcal{M}_3$ & 196.12 & 7.82 & 90.51 & 278.81 & 86.55 \\ 
%%  EWA & 176.30 & 11.28 & 113.79 & 278.81 & 71.98 \\ 
%%  OWA & 176.16 & 10.21 & 112.86 & 278.81 & 72.63 \\ 
%%  E-OWA & 173.25 & 6.41 & 111.97 & 278.81 & 113.43 \\ 
%%  $\mathcal{M}_F$ & 112.80 & 9.85 & 175.87 & 278.81 & 174.25 \\   
%   $\mathcal{M}_1$ & 187.80 & 9.70 & 100.72 & 278.81 & 82.83 \\ 
%  $\mathcal{M}_3$ & 197.03 & 12.81 & 94.59 & 278.81 & 73.27 \\ 
...
%\marginpar{It may be worth discussing the variance of the different aggress compares to the variance of Y}

%\subsection{People's Weight}
%This data is very good because it can be thought of living in the same probability space and also there are a lot of forecast per problem which should revealed our theoretical results better. 
%
%\begin{figure}[t!]
%   \centering
%   \includegraphics[width = 0.7\textwidth]{App1Data} % requires the graphicx package
%   \caption{}
%   \label{fig:example}
%\end{figure}
%
%
%\subsection{Business School 3rd-Year Salary}
%\begin{figure}[t!]
%   \centering
%   \includegraphics[width = 0.7\textwidth]{App2Data} % requires the graphicx package
%   \caption{}
%   \label{fig:example}
%\end{figure}
...
%analysis of the revealed aggregator should continue with the hope of finding more 
%
% Ideally, the aggregator is reliable and expanding. Developing such an aggregator would certainly be an important contribution to 


% or the observables are not stationary enough to allow accurate extrapolation outside the training set.  


%Learn more properties about the revealed aggre. 
%
%We need to find an aggregator that is expanding. Perhaps a good way to start is to look for a marginally consistent estimator first that is expanding. Develop intuition what this class looks like and then find a calibrated one within that set. ... maintaining calibration seems like a hard problem. How can we pinpoint such an aggregator.

%\marginpar{finding good aggregators continues. Separate problems of bias and noise. Solve the problem of bias reduction first. }
%
%\marginpar{interesting to relate to our JASA paper that shows that more extremiazing is needed when there is high information overlap. }
