...
Facing such high variability among the individual predictions, whose advice should Panette follow? Perhaps that of the most knowledgeable analyst? Unfortunately, determining the most knowledgeable analyst ex-ante is often not possible, and even if Panette could somehow do it, 
%ex-ante which analyst will be the most accurate. 
this or any other analyst is unlikely to know everything that the other analysts know. Therefore simply following a single analyst's advice 
%The idea is that even  only the most accurate forecaster's advice 
would ignore a potentially large amount of relevant information that is being contributed by the rest of the group. A better alternative is to somehow aggregate the predictions into a single consensus that represents the analysts' combined information. An aggregator that achieves this without leaving any information on the table is called \textit{efficient}. 
%Alternatively, the aggregator can be seen to \textit{collect information} from all the forecasts and then use all of it to make a consensus prediction.
%Clearly, to do this, the aggregator must somehow capture and understand the information among the analysts.

%The agents' disagreement, however, should not be seen as a hindrance but rather as an opportunity to capture the \textit{wisdom of the crowds} \citep{surowiecki2005wisdom}. To do this, Leon Penetta would only need to combine the predictions. For instance, the most common approach would simply average the forecasts. This has 

There are, of course, many different ways one can go about combining predictions. In practice, most people would probably first consider taking the average. 
% Panette could combine the predictions in many different ways. Most people would probably first consider the simple average. 
 Fortunately, this has been shown to perform remarkably well in a variety of settings (see, e.g., \citealt{clemen1986combining, clemen1989combining, makridakis1983averages})
% As long as the forecasts bracket the true outcome, that is, the outcome falls within their convex hull, averaging
and is often more accurate than the average individual \citep{larrick2006intuitions}. Other popular aggregators can be found in the general class of measures of central tendency. This class contains well known measures such as the median, geometric mean, and weighted average. While it is generally unclear which approach should be preferred, the forecasting literature by and large agrees that  aggregation of forecasts is beneficial. 
...
Two decades after writing this paper, the Nobel laureate C. W. J. Granger published an invited review, emphasizing the importance of forecasters' information in both theory and interpretation of forecasts \citep{granger1989invited}. 
...
% Therefore different parts of information can be shared by different subgroups of the forecasters, and 
 This way the forecasters' information can form a very complex network of partially overlapping sets. 
...
Analyzing aggregation across all possible information structures may at first appear combinatorially intractable. This, however, is not so. In fact,
...
%In fact, some authors have analyzed the average forecast without restricting the forecasters' information structure. 
 \cite{dawid1995coherent} analyze probability forecasts of a future event and show that the weighted average of two forecasts is inefficient under any information structure. 
...
\cite{Ranjan08} generalize this result to $N \in \mathbb{N}$ probability forecasts of a future event. 
...
In addition to not restricting the information structure, these authors assume nothing about the underlying probability model except its existence. Therefore their results are extremely general and hold jointly across all probability models and information structures.
...
%These results are very general because  the authors do not restrict the forecasters' information structure or assume anything about the underlying probability model except its existence. 



%\marginpar{While the Dawid and Gneiting did perform a fully general analysis it perhaps was not as clear because they do not explicitly notate the information sets. }
...
This paper continues the analysis and generalizes the inefficiency results in two different dimensions. First, 
...
%Therefore our results are more general that previous studies in more than one dimension. 
 The following list gives a more detailed explanation of our contributions.
...
ADD: It is perhaps because of this that the average is often criticized for representing the shared information over and over, leading to what is known as the \textit{shared-information bias} \citep{palley2015eliciting}. 


\begin{enumerate}[a)]
\item Section \ref{propertiesS} explains that an aggregator is efficient if and only if there exists some probability model under which the aggregator uses all of the forecasters' information.
...
\item Section \ref{TranscontractionSection} considers transformed weighted averages that apply a strictly monotone transformation to the forecasts, takes a weighted average of the transformed forecasts, and then transforms the average back to the original space. The results show that if the forecasts are non-negative, then this aggregator is not efficient.  
...
Therefore well-known aggregators, such as the harmonic, geometric, quadratic, and cubic (weighted) means, are never efficient. In addition, it is not efficient to average the log-odds or the probit scores of probability forecasts. Interestingly, \cite{dawid1995coherent} conjecture that the general class of transformed weighted averages of two probability forecasts is inefficient. The main theorem in this section now shows his conjecture to be correct. 
...
To illustrate the real-world implications of these results, consider the 2016 presidential elections in the United States. Many polling companies were not able to predict the Republicans' victory with good confidence. For instance, only a few days before the election day, the highly-celebrated polling analysis site FiveThirtyEight predicted a 30\% chance of Donald Trump winning. Why is this so low? The user's guide to Nate Silver's prediction algorithm \citep{silver} reveals that the state-specific polls are aggregated in two steps: a) correct biases (convention bounce bias, likely voter bias, etc.) by adjusting the polls' predictions, and b) take the weighted average of the adjusted predictions. Silver does not fully disclose how the weights are computed. This, however, does not matter because all weighted averages are inefficient. In other words, some relevant information provided by the polls was left on the table in each state. Efficient aggregation would have likely assigned Trump a higher chance of winning. Would this have changed the way the candidates or the voters behaved and hence led to a different result? Unfortunately, we will never know. 
...
Consider some probability model $(\Omega, \F , \P)$ and all measurable random variables defined $\Omega$. Those random variables that have a finite $p$th moment are collected in the space $\mathcal{L}^p  := \{ X \in \F : \E[|X|^p] < \infty\}$. Of particular interest is $\mathcal{L}^2$ that contains all square-integrable random variables. Given that $X \in \mathcal{L}^2$ implies $X \in \mathcal{L}^1$, the space $\mathcal{L}^2$ contains all random variables with finite means and variances. Now, denote the future outcome of interest with $Y$. Suppose that $Y \in \mathcal{L}^2$ and notate its mean and variance with
...
% probability space
%$(\Omega, \F , \P)$.
This outcome is defined on $(\Omega, \F , \P)$ that is a probabilistic description for the way $Y$ behaves in practice. The main results in this paper do not rely on the specific choice of $(\Omega, \F , \P)$ but instead apply to all probability models jointly. The only assumption is the existence of such a model. This, however, cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].''  
%The set $\Omega \neq \emptyset$ represents all possible states of nature and $\F = \mathcal{P}(\Omega) = \{A : A \subset \Omega\}$ is the set of all subset of $\Omega$. 

%\marginpar{This could also be said by $X \in L^2$; Define this with general $p$ and explain that $p1$ is contained in $p2$.}

Suppose there are $N$ forecasters predicting the value of $Y$. Denote the $j$th forecaster's prediction with the random variable $X_j \in \F$, $j = 1, \dots, N$. Relevant previous studies, including the aforementioned \citet{granger1989invited}, \citet{dawid1995coherent}, \citet{kim2001inefficiency}, and \citet{Ranjan08}, assume that 
%this paper assumes that 
the forecasters' predictions are based on correct but different information about $Y$.  Such forecasts are typically constructed by introducing a hidden random variable $Z_j$ whose value is observed by the $j$th forecaster. In any Bayesian setup, with a proper loss function, it is then essentially tautological that the $j$th forecaster predicts $X_j = \E(Y | Z)$. In some sense this is the ``easy'' case as it excludes external factors and makes the environment as amenable to information collection as possible. \citet{Ranjan08} in particular adopt the viewpoint that if an aggregator cannot collect information in such an ideal setting, it is unlikely do so after these forecasts have been biased or distorted with some false information or noise. 

\marginpar{Need to discuss this more; why does it make sense? It is hard to capture all biases. Is it?}
...
The partial information framework adopts this viewpoint but,
%To make this mathematically precise, it is necessary to introduce a general construction of information-driven forecasts. 
akin to many previous studies in statistics, finance, and economics, describes information directly in terms of $\sigma$-fields (see \citealt{herves2013information} for why $\sigma$-fields are a reasonable description of information) instead of a hidden random variable.  
%In the past many fields, including statistics, finance, and economics, have used $\sigma$-fields as a reasonable description of information
...
Given that $\E(Y|Z_j) = \E(Y | \F_j)$ for $\F_j = \sigma(Z_j)$, this formulation is essentially equivalent to the one used by the previous studies and described above. The only difference is that our formulation excludes the extra random variable $Z_j$ and hence makes it more clear that nothing is assumed about the sources of the information. For instance, the forecaster's information could stem from books, news articles, personal recollections, and so on. These details are not relevant for aggregation and hence have been abstracted away.
...
The following proposition shows that forecasts are information-driven, that is, they can be expressed as conditional expectations of $Y$ if and only if they are calibrated.
...
% The aggregator only operates on the reported forecasts and hence can access only
  information these forecasts reveal. Therefore, for the sake of aggregation, any unused information may as well not exist, and it is in no way restrictive to set $\F_j = \sigma(X_j)$, namely the $\sigma$-field generated by the forecast. 
...
An aggregator $\mathcal{X}''$ is called efficient if there exists a probability space $(\Omega, \F, \P)$ under which $\mathcal{X}'' = \E(Y | X_1, \dots, X_N)$ or, equivalently, $\mathcal{X}'' = \E(Y | \F'')$ with $\F'' = \sigma(X_1, \dots, X_N)$. 
...
\cite{dawid1995coherent} call such pairs \textit{compatible} and give some results on how to construct one from the other. Our focus, however, is not on any specific pair. Instead, we aim to investigate whether there exists a probability space under which some measure of central tendency is efficient. If there is not, then these aggregators do not satisfy the condition in Definition \ref{infoAgg} and can be deemed \textit{inefficient}.
...
Our path to such results begins with the following theorem that 
%In order to better understand efficient aggregators, the next theorem
 enumerates some of properties of efficient aggregators.
...
According to item ii), $\mathcal{X}''$ is calibrated. This property follows directly from Proposition \ref{calibrationProp} because, by definition, $\mathcal{X}''$ is based on $\F''$. 
...
The following example uses a general parametric model to provide intuition on why the weighted average cannot be efficient.
% goes wrong with direct averaging of forecasts. 

\begin{example}
 \cite{satopaamodeling} introduced the Gaussian partial information model as a close representation of the general partial information framework. This model consists of a centered Gaussian process $\{ X_B \}$ that is indexed by the 
...
Consider two forecasts $X_1$ and $X_2$. Their forecasts can be additively decomposed based on private and public information: $X_j = X_{B_j / B_i} + X_{B_i \cap B_j}$. If one had access to the forecasters' private information sets $B_1$ and $B_2$, the optimal aggregator would be $\mathcal{X}' := X_{B_1 \cup B_2}= X_{B_1 / B_2} + X_{B_2 / B_1} + X_{B_i \cap B_j}$.  \cite{satopaamodeling} call this aggregator the \textit{oracular aggregator} because it relies on information that is not typically available in practice, namely the forecasters' information sets $B_1$ and $B_2$. Given that aggregation cannot be improved beyond $\mathcal{X}'$, the oracular aggregator is the theoretical optimum. In practice, however, the aggregator can only access information revealed by the reported forecasts. All this revealed information is used by the efficient aggregator $\mathcal{X}'' = \E(Y | X_1, X_2)$. Unlike the oracular aggregator, $\mathcal{X}''$  can be applied in practice and therefore represents the practical optimum.
...
Now, consider the weighted average $\mathcal{X}_w = wX_1 + (1-w)X_2 = wX_{B_1 / B_2} + (1-w)X_{B_2 / B_1} + X_{B_i \cap B_j}$.  The sub-optimality of  the weighted average is sometimes explained with the fact that it represents the shared information too many times \citep{palley2015eliciting}. Comparing $\mathcal{X}_w$ to the oracular aggregator $\mathcal{X}'$, however, shows that $\mathcal{X}_w$ represents the public information $X_{B_i \cap B_j}$ exactly as it should but under-represents the forecasters' private information, $X_{B_1 / B_2}$ and $X_{B_2 / B_1}$. Therefore its sub-optimality does not arise from over-representation of the public information but from under-representation of the private information. 

To compare $\mathcal{X}_w$ with the efficient aggregator $\mathcal{X}''$, suppose, for the sake of simplicity, that each forecasters knows equally much: $|B_1| = |B_2|$. The amount of information overlap $|B_1 \cap B_2|$ is still left unknown. 
...
%
Then, $\mathcal{X}'' = \gamma X_1 + \gamma X_2 = \gamma X_{B_1 / B_2} + \gamma X_{B_2 / B_1} + 2\gamma X_{B_i \cap B_j}$, where $\gamma = |B_1| / (|B_1| + |B_1 \cap B_2|) \in [1/2, 1]$.  The ratio of weights from public to private information is $2\gamma / \gamma = 2$. This is exactly the ratio that an equally weighed average would have. 
%and let $\F_i := \sigma (X_{B_i})$.  Let $p_i := \E (\one_A \| \F_i)$.
Therefore $\mathcal{X}''$ represents the private and public information in the same proportion as an equally weighed average would, yet it is efficient. The difference is that $\mathcal{X}''$ assigns more weight in total than the average does. More specifically, the average assigns a total of weight of $w + (1-w) + 1 = 2$ to $ X_{B_1 / B_2}$, $ X_{B_2 / B_1}$, and $ X_{B_1 \cap B_2}$ whereas $\mathcal{X}''$ assigns a total weight of $4\gamma \in [2,4]$. 
...
% Therefore, if the total weight is represented with a pie, public and private information receive slices that are proportionally the same size under $\mathcal{X}''$ and $\mathcal{X}_w$ but the pie itself is smaller under $\mathcal{X}_w$. 
 
To understand why this hurts $\mathcal{X}_w$, note that an aggregator of the form $a X_1 + b X_2$ cannot equal the oracular aggregator $\mathcal{X}'$ under all information overlaps.  This is because if it represents all private (public) information appropriately, it ends up over-representing the public information (under-representing the private information). Therefore there is a trade-off in over-representing public and  under-representing private information.  

The weighted average $\mathcal{X}_w$ always represents the public information appropriately and hence under-represents the private information. Intuitively, however, it seems that some form of a compromise would lead to improved performance. This is precisely what $\mathcal{X}''$ does. Given that $\gamma \in [1/2, 1]$, it both over-represents public information and under-represents private information (except when $\gamma$ is on the boarder of $[1/2, 1]$. But it does this in a way that brings it as close (in terms of expected squared difference) to the oracular $\mathcal{X}'$ as possible. The above choice $\gamma$ indeed minimizes $\E [(\gamma X_1 + \gamma X_2 - \mathcal{X}')^2]$. Therefore, perhaps somewhat counter-intuitively, over-representing public information is beneficial. The weighted average can never do this and therefore remains inefficient. 
...
This section expands Definition \ref{Xw} to allow transformations of the forecasts before they are averaged. Typically the goal is to transform the forecasts $X_j$ to some other space where they are more symmetric around their mean which, in this case, is assumed to be some ``ideal'' forecast \citep{baron2014two}. After averaging, the result is transformed back to the original space. This gives us our next class of aggregators: 
...
\cite{dawid1995coherent} mention this class of aggregators. They focus largely on the case with only two forecasters and conjecture that $\mathcal{X}_{\Phi}$ can be efficient only if one of the forecasters knows everything that the other forecaster knows. This conjecture supposes that $\Phi$ is a continuous monotonic function.
...
The next example, however, shows that this is not sufficient for general inefficiency results. In addition, the behavior of $\Phi(X_j)$ must be restricted in some way. 

\begin{example} \label{ExampleTransf}
Let $\Omega = \{\omega_1, \omega_2, \omega_3, \omega_4\}$. The outcome is $Y = \one_A$, where $A = \{\omega_3, \omega_4\}$. Consider two forecasters with the information sets $\F_1 = \sigma(\{\omega_1\}, \{\omega_2\})$ and $\F_1 = \sigma(\{\omega_3\}, \{\omega_4\})$. Thus no forecaster is redundant, that is, their joint information is more than either of the individual information sets. Suppose $\P(E) = |E|/4$ for all $E \in \F$. Then the forecasts are
\begin{align*}
...
Now, consider $\mathcal{X}_\Phi$ with $\Phi(x) = \logit(x) =  \log(x / (1-x))$. Notice that $w\logit(X_1) + (1-w)\logit(X_2) = \logit(Y)$ for all $w \in (0,1)$.  This holds because one of the log-odds equals to $\logit(Y) \in \{-\infty, \infty\}$ while the other one is zero. In either case multiplying by a weight constant has no effect. Thus averaging here has no effect. After transforming the average log-odds back to the probability space, we have that 
...
This Example shows that $\mathcal{X}_\Phi$ can ``ignore'' the averaging effect if $\Phi(X_j)$ can be infinitely large.
...
Given that the main purpose of our analysis is to understand averaging, assume that $\Phi(X_j) \in \mathcal{L}^2$ for all $j = 1, \dots, N$. This makes sure that $\Phi(X_j)$ is almost surely finite and has finite mean and variance. This way averaging behaves as one would expect, leading to a meaningful analysis. The following theorem operates under this assumption and shows that many transformed weighted averages are in fact inefficient. 
...
Suppose $\Phi$ is strictly monotone, $\mathcal{X}_\Phi$ assigns positive weight to at least two different forecasts, and assume $\Phi(X_j) \in \mathcal{L}^2$ for all $j = 1, \dots, N$. Then the following holds.\begin{enumerate}[i)]
\item If $\Phi$ is strictly convex or concave, then  $\mathcal{X}_\Phi$ is not marginally consistent: $\E[\mathcal{X}_\Phi] \neq \mu_0$.
\item If $\Phi$ is convex or concave, then $\mathcal{X}_\Phi$ is not calibrated: $\P(\E[Y | \mathcal{X}_\Phi] \neq \mathcal{X}_\Phi) > 0$. Furthermore, this shows that $\mathcal{X}_\Phi$ is not efficient.  \label{convex} 
\item If  $X_j \geq 0$ and there exists a finite constant $B_\partial$ such that $\big| \frac{\partial }{\partial x}  \Phi^{-1}(x)\big| \leq B_\partial$ for all $x$ in the domain of $\Phi^{-1}$, then $\mathcal{X}_\Phi$ is not efficient. \label{mono}
\end{enumerate}
\end{theorem}
...
many well-known aggregators. For instance, the \textit{weighted generalized means} of non-negative forecasts are given by 
\begin{align*}
\left\{ \mathcal{X}_a := \left( \sum_{j=1}^N w_j X_j^a \right)^{1/a} : a \in \R, w_j \geq 0, \sum_{j=1}^N w_j = 1 \right\},
\end{align*}
%where each $X_j \geq 0$ and $a \in \R$ is a given constant. 
where the function $\Phi(x) = x^{a}$ is concave if $a \in [0,1]$ but convex otherwise. Notable members of this class include the harmonic, geometric, arithmetic, quadratic, and cubic means, corresponding respectively to $a = -1, 0, 1, 2, 3$. By item \ref{convex}) of Theorem  \ref{contractionXphi}, however, no generalized mean is efficient as long as it assigns positive weight to at least two different forecasts. In fact, it is not even calibrated and hence based on any information set about $Y$.
...
\begin{table}[t!]
   ...
   \hline
     & Distribution & $F^{-1}(x)$ & Range for $f(x)$  \\ \hline
& Log-logistic $(\alpha = \beta = 1)$ & $\odds(x) = x/(1-x)$ & $[0,1]$\\
& Logistic $(\mu = 0, s = 1)$ &$\logit(x) = \log(x/(1-x))$  & $[0,1/4]$\\
& Gaussian $(\mu = 0, \sigma^2 = 1)$ & $\probit(x)$ & $[0,1/\sqrt{2 \pi}]$\\ \hline
        \end{tabular}
   \caption{Popular transformations $F^{-1}$ of probability forecasts are quantile functions of well-known distributions. The inverse function $F$ is then the cumulative distribution function whose derivative is probability density function. This is bounded for many distribution, satisfying the conditions of item \ref{mono}) in Theorem  \ref{contractionXphi}}
   \label{distrTable}
\end{table}

%Second, consider the 
Transformed weighted averages are particularly popular when making probability forecasts
%Practitioners, however, do use $\mathcal{X}_\Phi$ with a non-convex (or non-concave) $\Phi$.
 $X_j \in [0,1]$ of a binary outcome $Y \in \{0,1\}$. Here transforming the forecasts is useful when the outcome is highly unlikely or likely because in such cases the distribution of the forecasts is often skewed towards the middle. Consequently, the direct average is going to be too close to $0.5$. This problem can be somewhat mitigated by transforming the probabilities before averaging. Empirically this has been observed to give better performance than simply averaging the forecasts \citep{baron2014two, satopaa, satopaamodeling2}.  
...
A popular choice for $\Phi$ is the quantile function $F^{-1}$ of some distribution. 
...
By the Leibniz integral rule, the derivative $\frac{\partial}{\partial x} F(x) =  f(x)$ is the probability density function (PDF). This is non-negative and upper bounded for many distributions. For instance, Table \ref{distrTable} lists some of the most popular transformations, namely the odds, log-odds, and probit, their corresponding distributions, and ranges for their PDFs. 
...
Unfortunately, by item \ref{mono}) of Theorem  \ref{contractionXphi}, none of these transformations and many other distributions lead to an efficient version of $\mathcal{X}_\Phi$. 
...
Even though the results discussed so far appear to capture most of the commonly used aggregators, they do not apply to many well-known measures of central tendency, such as the median, midrange, or trimmed averages.  In short, this is because such aggregators do not treat the forecasts symmetrically ex-ante but instead depend on their realized values. 
...
The key towards a fully general analysis is to observe that all such measures are constrained to the convex hull of the forecasts. This property defines a new aggregator class that is analyzed in the next section.
...
Therefore, even though a forecaster with infinite information may be a convenient approximation in some theoretical work, it is not a precise description of reality. This gives us the following corollary.
