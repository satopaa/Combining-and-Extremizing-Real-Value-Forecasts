...
close to the marginal event frequency and hence considered under-confident.  This result is an important
...
framework} as a general platform for combining forecasts. Their framework operates under the same assumptions as the work by \cite{Ranjan08} but provides concrete models with the optimal aggregators. 
...
For instance, the authors describe a specification, called the Gaussian partial information model, under which optimal aggregation is based on the estimated covariance matrix of the individual forecasts. They apply this model to different types of synthetic and real-world data, and illustrate how it improves upon averaging and other types of measures of central tendency. 
%
...
The main contribution of this paper uses the general partial information framework to extend the result
...
is both unreliable and under-confident. The analysis
provides general, yet intuitive, descriptions of well-known properties
such as reliability and resolution. The discussion also introduces a new property
...
\textit{Extremizing} is the act of transforming a forecast away from the non-informative prior probability of $0.5$. This technique is increasingly used in the probability forecasting literature as a means of
alleviating the systematic under-confidence of the average forecast. For instance, \cite{Ranjan08} propose a beta transformation that extremizes the weighted average of
...
%  the probability forecasts; \cite{satopaa} use a logistic regression model to extremize the average log-odds of the
%  forecasts; \cite{baron2014two} propose yet another extremizing  transformation.}
Conceptually, extremizing is a very simple idea that can be applied to any kind of forecasts -- not just to probability forecasts. This paper in particular suggests that extremizing is beneficial also when the forecasts are real-valued. 
...
More specifically, our second contribution shows on synthetic and real-world data  that transforming the weighted average of real-valued forecasts further away from the  naive baseline forecast, namely the marginal mean of the outcome
...
% all the following results, apart from the parameter estimates, represent out-of-sample performance. 
