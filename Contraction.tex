...
%would ignore a potentially large amount of relevant information held in the rest of the group. Therefore, to truly harness the potential of the group, Panette needs a way to make the decision based on the analysts'  joint information.
...
%Decision-makers often rely on the advice of experts. For instance, 
In early 2011 the U.S. intelligence believed the terrorist leader Osama Bin Laden to be hiding in a particular Pakistani compound. To decide whether to assault the compound or not, the CIA Director Leon Panetta was consulting a group of intelligence analysts. The agents, however, did not agree on the probability of Osama Bin Laden being in this compound. Their answers ranged from $0.30$ all the way to $0.90$ and above \citep{tetlock2016superforecasting}. 
%Based on such high disagreement, should the military proceed with the assault inside a volatile nation armed with nuclear weapons? 
Facing such wide disagreement, whose advice should Panette follow? Perhaps that of the most knowledgeable analyst? 
%Unfortunately, determining the most knowledgeable analyst ex-ante is often not possible, and even if Panette could somehow do it, 
%ex-ante which analyst will be the most accurate. 
...
%This way they do not need to be in the same location and can make their predictions on their own time without knowing anything about the others. 
% Regardless how this is done, t
 with a statistical aggregator that is nothing but a deterministic function from the predictions to  the consensus. This way the aggregator does not contribute any new information. Instead all information originates from the forecasters.  Given that the aggregator does not influence or have access to the internal mechanisms by which the individuals turn their information into overt predictions, it is the individual's responsibility to construct a prediction based on truthful and relevant information. Such ``honest'' predictions are considered to be \textit{information-driven}. The aggregator's responsibility, on the other hand, is to extract the maximum amount of information from the reported predictions and turn it into a consensus without any loss or distortion of information. If an aggregator achieves this, it is called \textit{efficient}. 
...
%Two decades later, the second author, 
...
After all, forecasting should be based on information about the future outcome -- not on instinct, random guessing, or gut feel. This observation led to different stylized models of information-driven forecasts. 
...
%The main modeling differences are in the assumptions made about the structure of the forecasters' information.
 For instance, \cite{granger1989invited} and \cite{kim2001inefficiency} consider models in which each forecaster's information consists in equal parts of shared information (known to everyone) and private information (known only to one forecaster) about the outcome. Surprisingly, both authors observe that the simple average of the forecasts is not efficient. In other words, it is not  optimal and hence can be improved upon. It is important, however, to note that this result, known as the \textit{inefficiency of the means}, was
...
%The main limitation of this work stems from its restrictive assumptions on the forecasters' information structure: all information is known either by everyone or by only one forecaster. Due to its tractability, this \textit{symmetric} information model has become the popular choice in the literature (see, e.g., \citealt{ottaviani2006strategy, lichtendahl2013wisdom, palley2015eliciting}).
 discovered under specific models with unrealistic assumptions on the forecasters' information structure. Therefore it does not imply general inefficiency.
%that averaging is always inefficient. 
 
 
 In reality the forecasters can have different amounts of information, and not all information is either entirely shared or private. For instance, the first two forecasters may share some information that the third forecaster does not know and so on. 
% Therefore different parts of information can be shared by different subgroups of the forecasters, and 
This way the forecasters' information can be seen to form a very complex structure of partially overlapping sets. Even though analyzing all such possible information structures may at first appear combinatorially intractable,
...
%This, however, is not the case.
  some previous authors have carried out such exhaustive analyses. More specifically, 
...
\cite{Ranjan08} generalize this result to $N \geq 2$ probability forecasts.
...
% and therefore apply to predictions of continuous (Gross Domestic Product, stock prices, product demand), binary (rain or no rain, candidate wins or loses), discrete (number of accidents, goals per season), and many other univariate types. 
 Second, the class of inefficient aggregators is shown to reach far beyond weighted averages. 
...
Theorem \ref{contraction} shows that any non-trivial weighted average violates several of the properties listed in Theorem \ref{optimal} and is inefficient. By non-trivial we mean that the average assigns positive weight to at least two different forecasts. More specifically, such a weighted average distorts the forecasters' information and hence leads to sub-optimal predictive performance.
% to suboptimal performance. 
%The section concludes by developing intuition for why weighted averaging is inefficient.  
In the past this result has been believed to arise because the average represents the forecasters' shared information too many times \citep{kim2001inefficiency,palley2015eliciting}. We, however, explain that this is not the reason. First, because the weights must sum to one, the weighted average cannot treat information consistently across different aggregation scenarios. Second,
%In short, because private and shared information cannot be separated from the predictions, 
any linear aggregator faces a trade-off between \textit{under}-representing private information  and \textit{over}-representing the shared information. The efficient strategy balances the trade-off by intentionally over-representing the common information in order to under-represent the private information less. 
...
\cite{satopaamodeling2, satopaamodeling} propose \textit{partial information framework} as a general platform for analyzing and interpreting information-driven forecasts. This framework operates under  
%, that forms the basis for our analysis of efficient forecast aggregation.
%To begin, consider 
some probability model $(\Omega, \F , \P)$. The goal is to predict the value of an outcome $Y \in \mathcal{L}^2$, where $\mathcal{L}^p  := \{ X \in \F : \E[|X|^p] < \infty\}$ is the set of all random variables $X \in \F$ with finite $p$th moment. Denote the mean and variance of $Y$ with 
...
$\E(Y) := \mu_0 < \infty$ and $\Var(Y) := \delta_0 < \infty$.
% under some
% probability space
%$(\Omega, \F , \P)$.
It is important to emphasize that even though the outcome $Y$ is defined on $(\Omega, \F , \P)$, our main results do not depend on any specific probability model but instead apply to all such models jointly. The only assumption is the existence of such a model. This, however, cannot be avoided in any probabilistic treatment of forecast aggregation. In fact, according to \cite{lindley} this is essential and ``any other approach has an element of adhockery about it [...].''  
%The set $\Omega \neq \emptyset$ represents all possible states of nature and $\F = \mathcal{P}(\Omega) = \{A : A \subset \Omega\}$ is the set of all subset of $\Omega$. 

%\marginpar{This could also be said by $X \in L^2$; Define this with general $p$ and explain that $p1$ is contained in $p2$.}

Suppose there are $N \in \mathbb{N}$ forecasters predicting $Y$. Denote the $j$th forecaster's prediction with the random variable $X_j \in \F$, $j = 1, \dots, N$. Like the previous studies discussed in Section \ref{Contributions},
...
Based on $\F_j$ the forecaster forms a conditional distribution $\P(Y|\F_j)$ of the outcome. Given that the predictions are assumed to be univariate, the forecaster must summarize $\P(Y|\F_j)$ with a single value. 
...
The final choice is ultimately determined by the loss function $L(X,Y)$ that is used for evaluating the accuracy of the prediction. To this end, forecasting applications typically use a \textit{Bregman loss function} defined as follows: if $\phi: \R \to \R$ is a strictly convex, differentiable function, then the Bregman loss function is $L(x,y) = \phi(x)-\phi(y) - (x-y)\phi'(y)$. The most well-known member of this class is the squared error loss $L(x,y) = (x-y)^2$ given by $\phi(x) = x^2$. In general, if $\phi$ is continuously differentiable, then Bregman loss functions are precisely those functions whose expected value is minimized by $\E(Y|\F_j)$ among all $\F_j$-measurable random variables, i.e.,  $\argmin_{X \in \F_j} \E[L(X,Y)] = \E(Y|\F_j)$ \citep{banerjee2005optimality}. This article focuses on minimizing the Bregman loss and hence supposes that the forecasters summarize $\P(Y|\F_j)$ as $X_j = \E(Y | \F_j)$  for $j = 1, \dots, N$. Even though many other choices could be made, the expected value is a very natural starting point. More importantly, as is explained in the rest of the section, past literature describes many practical ways in which the forecasters can be guided to report predictions of this form.
...
Calibration is the minimal condition that the forecasters must satisfy in order to fulfill their responsibility in collective forecasting. In fact,  a forecast $X$ is calibrated if and only if it is based on some partial information set, that is, $X = \E(Y|\G)$ for some $\G \subseteq \F$ \citep{satopaamodeling}. This is convenient because, instead of somehow trying to identify true information and turning that into a prediction, the forecasters can focus on the more concrete goal of being calibrated.
...
To illustrate, consider a calibrated probability forecast. If it equals $p$, then the outcome happens $100p\%$ of the time. This is precisely how probabilities are generally understood. Most forecasters with a basic understanding of probabilities are likely to target good calibration even without being instructed to do so. In fact, researchers have found many calibrated subpopulations of experts, such as meteorologists \citep{murphy1977reliability}, experienced tournament bridge players \citep{keren1987facing}, and bookmakers \citep{dowie1976efficiency}. On the more theoretical side, \cite{dawid1982well} show that an internally consistent or \textit{coherent} Bayesian can expect to become calibrated under feedback. \cite{foster1998asymptotic}, on the other hand, show that the forecaster needs to know almost nothing about the outcome in order to be calibrated. 
...
\cite{dawid1995coherent} call such pairs \textit{compatible} and give some results on how to construct one from the other. Our focus, however, is not on any specific pair. Instead our goal is to investigate whether there exists a probability space under which a given aggregator is efficient. If there is not, then this aggregator does not align with Definition \ref{infoAgg} and hence is always \textit{inefficient}. To show these kinds of inefficiency results directly with Definition \ref{infoAgg}, however, can be challenging. An easier approach is to first understand the common behavior of all efficient aggregators and then check whether a given aggregator, such as the weighted average or median, agrees with it.  
...
The next theorem describes this behavior by enumerating several common properties of all efficient aggregators.
...
an increasing sequence of $\sigma$-fields $\F_0 = \{\emptyset, \Omega\} \subseteq \F_1 \subseteq \dots \subseteq \F_R \subseteq \F$ and the corresponding forecasts $X_r = \E(Y|\F_r)$ for $r = 0,1, \dots, R$. \citet{satopaamodeling2} show that the variances of these forecasts follow the same order as their information sets, that is, $\Var(X_0) = 0 \leq \Var(X_1) \leq \dots \leq \Var(X_R) \leq \Var(Y) = \delta_0$. Thus the amount of information in a calibrated forecast is measured by its variance. Naturally, if an aggregator is efficient, it uses all of the forecasters' information and hence uses at least as much information as the most informed individual among them. Therefore the efficient aggregator $\mathcal{X}''$ is necessarily at least as variable as each of the individual forecasts. This is precisely what item iii) describes. The final item iv) shows that $\mathcal{X}''$ is the optimal aggregator under a large class of different loss functions and hence justifies efficiency as a goal in forecast aggregation. 
...
First, item \ref{XwCalib}) shows that $\mathcal{X}_w$ is almost never calibrated. Therefore it is not based on any correct information about $Y$ even though the forecasters only give it correct information. This means that weighted averaging somehow distorts the forecasters' information. 


Items \ref{underconfA}) and \ref{underconfB}) show that the distortion contracts $\mathcal{X}_w$ towards $\mu_0$, making it under-confident. The two items do this comparison under different benchmarks. Item \ref{underconfA}) defines and considers under-confidence relative to the calibrated version of  $\mathcal{X}_w$. Intuitively, this aggregator, called $\mathcal{X}_w''$, filters off false information and uses what ever correct information is left in $\mathcal{X}_w$. Given that $\mathcal{X}_w$ is closer to $\mu_0$ than $\mathcal{X}_w''$ is, the false information contracts $\mathcal{X}_w$ towards $\mu_0$. 
...
This inefficiency is sometimes believed to arise from the fact that $\mathcal{X}_w$ represents the shared information too many times \citep{kim2001inefficiency, palley2015eliciting}. The explanation, however, is more complicated than that. For one, the weighted average does not treat information consistently. To illustrate, consider two forecasters $A$ and $B$ who know equally much but their information sets do not overlap. In this symmetric case, any rational advocate of averaging would treat their predictions symmetrically and assign them equal weight. This way each unit of information receives weight $1/2$. Consider now forecaster $C$ who knows everything that $A$ and $B$ know. This time the advocate of averaging would assign weight one to $C$ and ignore $A$ and $B$.  Thus the same information, that earlier received weight $1/2$, now receives weight $1$. To resolve this conflict, the average would have to assign weight one to both $A$ and $B$ in the first scenario. Unfortunately, it cannot do this because the weights must sum to one. The next example explores this intuition in more detail. For the reader's convenience all of its technical derivations have been deferred to the Appendix.
% goes wrong with direct averaging of forecasts. 

\begin{example}\label{intuition}
%Without loss of generality, let $\Var(Y) = 1$. 
For intuition it suffices to consider only $N = 2$ predictions.  To understand how aggregators represent the private and shared information, our analysis must explicitly incorporate both types of information without making too many extra assumptions. To do this, consider the three-level hierarchy shown in Figure \ref{HierarchyPlot}. In the bottom of the hierarchy are three interns who each make predictions about $Y$. Suppose these predictions $X_{(1)}, X_{(1,2)}, X_{(2)}$ are independent. Therefore the interns use independent or ``disjoint'' sets of information. On the second level are two experts. Their private and shared information are determined by the interns' predictions. More specifically, the $j$th expert predicts $X_j = \E(Y|X_{(j)}, X_{(1,2)})$, where $X_{(j)}$ represents the private information and $X_{(1,2)}$ represents the shared information. In the end, the experts report their predictions $X_1$ and $X_2$ to the decision-maker who combines them with the weighted average $\mathcal{X}_w$. The objective is to explore how $\mathcal{X}_w$ represents the experts' different parts of information, namely $X_{(1)}, X_{(1,2)}, X_{(2)}$. 
...
% The weighted average $\mathcal{X}_w$ can be efficient only under some probability model where the efficient aggregator is linear, that is,
 Suppose $\mathcal{X}'' = \beta_1X_1 + \beta_2X_2$ for some constants $\beta_1, \beta_2 \in \R$.  
...
If $\delta_j := \Var(X_j)$ and $\rho := \Cov(X_1, X_2)$, then the coefficients can then be written as  %By minimizing the expected quadratic loss and using the fact that $\Cov(X_j, Y) = \delta$ (see \citealt{satopaamodeling} for more details), it is possible to show that
 $\beta_j = (\delta_1\delta_2 - \rho \delta_i)/(\delta_1\delta_2 - \rho^2)$, where $j,i \in \{1,2\}$ and $j \neq i$. This shows that if the experts' information sets $\F_1$ and $\F_2$ are independent, $\rho = 0$ and $\mathcal{X}'' = X_1 + X_2$. In other words, independent information combines via simple addition. To give intuition for this, consider two real-estate agents $A$ and $B$ who are valuing an apartment. Agent $A$ only sees one room and based on it values the apartment at $\$ x_A$. Agent $B$, on the other hand, sees a different room and  gives a valuation of $\$x_B$. If the agents know nothing else about the apartment except the rooms they visited, their information sets are disjoint and their combined information gives a valuation of $\$(x_A + x_B)$. 
%  combined 
 
 
 Now, given that the interns' information sets are independent, the $j$th expert's prediction can be written as $X_j = X_{(j)} + X_{(1,2)}$. By plugging this decomposition into the aggregators, they can be expressed in terms of private and shared information: 
 \begin{align}
...
\end{align}
%This shows 
Both aggregators assign the same proportion of weight to private and shared information. Their only difference is that the weights in $\mathcal{X}''$ are not sum-constrained. 
...
Therefore $\mathcal{X}_w$ can be efficient only when one of the expert's information is entirely contained within the other expert's information set. This happens if $X_{(1)}$, $X_{(2)}$, or both are identically zero. For these three cases the weights $(w_1, w_2)$ are $(0,1)$, $(1,0)$, and $(1/2, 1/2)$, respectively.  Given that in the third case the forecasts are equal, in none of these cases $\mathcal{X}_w$ is non-trivial. Therefore this observation aligns with item \ref{underconfB}) of Theorem \ref{contraction}. In all other cases there is partial information overlap and $\mathcal{X}_w$ could be improved by relaxing its sum-constraint. 
...
in a way that brings it as close to the theoretical optimum $\mathcal{X}'$ as possible. In fact, $\mathcal{X}''$ is the orthogonal projection  of $\mathcal{X}'$ onto the space of aggregators with the form $\beta_1X_1 + \beta_2X_2$.
...
Suppose $\Omega = \{\clubsuit, \heartsuit, \diamondsuit, \spadesuit\}$ and let $\F = 2^\Omega$ be all subsets of $\Omega$. The outcome is $Y = \one_A$, where $A = \{\heartsuit, \spadesuit\}$, and $\P$ is uniform over $\Omega$, that is, $\P(E) = |E|/4$ for all $E \in \F$. Consider two forecasters with the information sets $\F_1 = \sigma(\{\clubsuit\}, \{\heartsuit\})$ and $\F_2 = \sigma(\{\diamondsuit\}, \{\spadesuit\})$. In other words, the first forecaster knows whether the suit is Clubs or Hearts. The second forecaster, on the other hand, knows  whether the suit is Diamonds or Spades. Thus no forecaster is redundant, and their joint information is more than either of the individual information sets. In fact, together they know everything $\F'' =  \sigma(\{\clubsuit\}, \{\heartsuit\}, \{\diamondsuit\}, \{\spadesuit\}) =  \F$. The relevant predictions are
\begin{align*}
...
Now, consider $\mathcal{X}_\Phi$ with $\Phi(x) = \logit(x) =  \log(x / (1-x))$. This is an invertible function that gives $w\logit(X_1) + (1-w)\logit(X_2) = \logit(Y)$ for all $w \in (0,1)$.  The identity holds because one of the log-odds always equals to $\logit(Y) \in \{-\infty, \infty\}$ while the other one is zero. Multiplying either value by a constant has no effect, and $w$ could as well be removed from the aggregator. After transforming the average log-odds back to the probability space, the aggregator 
...
\item If  $X_j \geq 0$ for all $j = 1, \dots, N$ and $\Phi^{-1}$ has a bounded first derivative, then $\mathcal{X}_\Phi$ is not efficient. 
...
Unfortunately, by item \ref{mono}) of Theorem  \ref{contractionXphi},  these and many other CDFs do not yield an efficient $\mathcal{X}_\Phi$. 
...
Now, recall from Section \ref{outcomes} that $\F_j = \sigma(X_j)$ is generated by a random variable. This means that $\F_j$  is countably generated (see, e.g., \citealt{durrett2010probability}) and  
...
If all measures of central tendency are inefficient, the natural question to ask is whether an efficient aggregator even exist. Fortunately, the answer is ``yes.''  In fact, there are at least two different approaches to constructing such aggregators. The first approach assumes a functional form for the aggregator and then estimates it by minimizing the expected Bregman loss. For instance, if the efficient aggregator is a linear function of the forecasts, then $\mathcal{X}'' = \mu_0 + \diag(\bSigma)'\bSigma^{-1}(\boldsymbol{X}-\mu_0 \one_N)$, where $\boldsymbol{X} = (X_1, \dots, X_N)$ and $\bSigma := \Cov(\X,\X)$ satisfies $\delta_0 -  \diag(\bSigma)'\bSigma^{-1}\diag(\bSigma) \geq 0$. For more details on the derivation and how to estimate $\bSigma$ in practice, see \cite{satopaamodeling}.
...
The second approach does the reverse. In particular, it assumes a parametric form for the joint distribution of the outcome and its information-driven forecasts. 
% recommend a \textit{top-down}-approach that views aggregation as data analysis. The first step is to link the forecasts and outcomes under a single model. 
 The efficient aggregator $\mathcal{X}''$ is then found  by either analytically  or  numerically estimating $\E(Y|X_1, \dots, X_N)$ under the chosen model.
...
Our discussion has many links to other areas of forecasting. For one, Theorem \ref{contraction} shows and Example \ref{uncountable} illustrates that the simple weighted average is typically too close to the marginal mean $\mu_0$. Thus its performance could be improved by transforming it directly away from $\mu_0$. This is precisely what a post-transformation heuristic known as \textit{extremization} does. Extremization has been particularly successful in probability forecasting. 
%In probability forecasting the under-confidence of a simple aggregator, such as the average or median, is typically alleviated by a heuristic known as \textit{extremizing}, that is, by systematically transforming the aggregate towards its nearer extreme (at zero or one). 
%away from the naive baseline forecast, namely the marginal mean of the outcome. 
%non-informative prior probability of $1/2$
%This technique has become known as \textit{extremizing}. 
...
%To illustrate, observe that the forecaster with partial information $\F_j \subseteq \F$ has a conditional distribution $\P(Y | \F_j)$.
 In this paper the forecaster reports the expected value $\E(Y|\F_j)$ of $\P(Y | \F_j)$. Even though this is the natural starting point, it would be interesting to analyze other choices such as the median or $95\%$ credible interval. One could also examine the efficiency of popular techniques to capture other aspects of the outcome. For instance, the \textit{Newsvendor model}, that is widely used in operations management and applied economics, requires an estimate of  the variability of the outcome \citep{stevenson2002operations}. This is often estimated with the sample variance of the individual forecasts $\mathcal{S}_N = \sum_{j=1}^N (X_j - \bar{X})^2/(N-1)$. Does this use all of the forecasters' information and hence equal to $\Var(Y|X_1, \dots, X_N)$ under some probability model? Intuitively, given that $\Var(X_j) \leq \Var(Y)$ for all $j = 1, \dots, N$, the sample variance $\mathcal{S}_N$ is likely to underestimate $\Var(Y)$. 
 More specific nature of the bias could be explored by extending the common notion of (expectation) calibration to variance calibration as follows: an estimate $\mathcal{S}$ of $\Var(Y)$ is calibrated if $\Var(Y|\mathcal{S}= s) = s$ for all $s \geq 0$.
...
% 
  In terms of other applications, our results could shed light on estimation of tail risk in asset management or provide new insight on prediction or stock markets that are commonly believed to reflect all publicly available information. 
...
Let $\X = (X_1, \dots, X_N)'$. If $\Cov(\X,\X) := \bSigma$,  then it is well-known that $\argmin_{\bBeta} \E(Y- \bBeta\X)^2 =  \Cov(\X, Y) \bSigma^{-1} =: \bBeta^*$ , where $\Cov(\X, Y) = (\Cov(X_1,Y), \dots, \Cov(X_N,Y))$. Now, \cite{satopaamodeling2} show that if $X_j$ is calibrated, then $\Cov(X_j, Y) = \Var(X_j)$. Therefore $\bBeta^* = \diag(\bSigma)\bSigma^{-1}$. If $\Cov(X_j,X_i) = 0$ for all $i \neq j$, then $\bSigma = \diag(\bSigma)$, $\bBeta^* = \one_N'$, and $\bBeta^* \X = \sum_{j=1}^N X_j$. Therefore independent information combines via simple summing. 

Consider now the case of $N=2$ forecasters. Then, $\bBeta^* = \diag(\bSigma)\bSigma^{-1} = (\delta_1\delta_2 -\rho\delta_2, \delta_2\delta_2 -\rho\delta_1) / (\delta_1\delta_2 - \rho^2)$. The sum $\beta_1 + \beta_2 = (2\delta_1\delta_2 - \rho\delta_1 - \rho\delta_2) / (\delta_1\delta_2 - \rho^2)$. Given that
\begin{align*}
...
\begin{align*}
\min (\beta_1 + \beta_2) &= (\beta_1 + \beta_2) |_{\rho = \beta_1} = \frac{2\delta_1\delta_2 - \delta_1^2 - \delta_1\delta_2}{\delta_1\delta_2 - \delta_1^2} = 1.
\end{align*}
...
Furthermore, given that each $\Phi(X_j) \in \mathcal{L}^2 \subseteq \mathcal{L}^1$, where $ \mathcal{L}^p := \{X : \E[|X|^p] < \infty\}$, we have that
\begin{align*}
...
%are well-defined. This shows that each $\mathcal{X}''(C_k)$ is upper and lower bounded by $\mathcal{X}''(C_{k'})$ and $\mathcal{X}''(C_{k''})$
 where $\P(C_{k'}) > 0$, $\P(C_{k''}) > 0$, $C_{k'} \neq C$, and $C_{k''} \neq C$. Thus, if the forecasts do not all agree on some $C$, then $\mathcal{X}''(C)$ is strictly upper and lower bounded by some of its other values.  Furthermore, these values are linked to $C$ via some $A_{ij} \in \tau_j$, that is, there are some $A_{ij}'$ and $A_{ij}''$ such that $C \cup C_{k'} \subseteq A_{ij}'$ and $C \cup C_{k''} \subseteq A_{ij}''$. 
